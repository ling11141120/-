...
, z2 as (
    select *
      from (select '海剧'  as identifier
                 ...
                 , user_type
                 , user_value
                 , case when chl = 'moboreels-Android-GP' then '谷歌'
     ...
     where 1 = 1
          ...
          ${if(len(渠道来源) == 0,"","and case when source_chl in ('fbgroup','fbpage','fbpost','social') then '社媒'
                                              when source_chl in ('unattributed','google-play','organic','(not set)','none') then '自然量'
                                              when source_chl in ('exlink','officialsite','pinterest','podcasts','rss','webshare','seoyt') then 'SEO媒体'
                                              when source_chl in ('facebook','fbs2s','tt','adwords','mediago','appleadservice','sem','mapit','tiktok app','applovin','kwai') then '广告媒体'
                                              when source_chl = 'pwa' then 'pwa'
...
, z3 as (
    select *
      from (select '国剧' as identifier
                 ...
                 , null    as `国家`
                 , case when reg_language = '1' then '中文'
                        else '其他'
                    end    as `注册语言`
                 , user_type
                 , user_value
...
select dt
     ...
     , consume_wide_all_cnt
     , ad_amt
     , ifnull(natural_consume_money, 0) + ifnull(natural_presented_money, 0) as `非引流消耗金额`
     , ifnull(natural_consume_money, 0) as `非引流阅币消耗金额`
     , week
  from ${if(identifier='海阅','z1', if(identifier='海剧','z2', if(identifier='国剧','z3','z1')))}
;