with z1 as (
    select *
      from (select '海阅'                    as identifier
                  ...
                  , user_value
                  , null                       as chl
                  , case when source_chl in ('fbgroup', 'fbpage', 'fbpost', 'social') then '社媒'
                  ...
                  , case when cast(period_week as int) < 10 then concat(substr(dt, 1, 4), '-0', period_week)
                         when cast(period_week as int) >= 52 and substr(dt, 6, 2) = '01' then concat(substr(date_add(dt, -30), 1, 4), '-', period_week)
                         else concat(substr(dt, 1, 4), '-', period_week)
                     end                       as week
              from (select *
              ...
              left join dim.dim_dic as dic_reglang -- 注册语言
...
, z2 as (
    select *
      from (select '海剧'  as identifier
                 ...
                 , country as `国家`
                 , dic_reglang.remarks '注册语言', user_type
                 , user_value
                 , case
              ...
              when source_chl in ('unattributed', 'google-play', 'organic', '(not set)', 'none') then '自然量'
              when source_chl in ('exlink', 'officialsite', 'pinterest', 'podcasts', 'rss', 'webshare', 'seoyt')
                  then 'SEO媒体'
              when source_chl in
                   ('facebook', 'fbs2s', 'tt', 'adwords', 'mediago', 'appleadservice', 'sem', 'mapit', 'tiktok app',
                    'applovin', 'kwai') then '广告媒体'
              when source_chl = 'pwa' then 'pwa'
              when source_chl = 'pwa_edm' then 'pwa_edm'
              else '其他'
                   end     as
                '渠道来源'
                 , coalesce(corever, '其他') 'core', source_chl
                 , dau
                 , recharge_cnt
                 , consume_money
                 , consume_pay_vip
     ...
     where 1 = 1
          ${if(len(平台) == 0,"","and  平台 in ('" + 平台 + "')")} ${if(len(国家等级) == 0,"","and  国家等级 in ('" + 国家等级 + "')")} ${if(len(注册语言) == 0,"","and  注册语言 in ('" + 注册语言 + "')")} ${if(len(core) == 0,"","and  core in ('" + core + "')")} ${if(len(渠道来源) == 0,"","and
  case
      when source_chl in ('fbgroup','fbpage','fbpost','social') then '社媒'
      when source_chl in ('unattributed','google-play','organic','(not set)','none') then '自然量'
      when source_chl in ('exlink','officialsite','pinterest','podcasts','rss','webshare','seoyt') then 'SEO媒体'
  ...
  end in ('" + 渠道来源 + "')")} ${if(len(source_chl) == 0,"","and  source_chl in ('" + source_chl + "')")}
        )
   , z3 as
    (select *
       from (select '国剧' as identifier
                  , dt
                  , period_type
                  , period_week
                  , case
      ...
      where 1 = 1
          ${if(len(平台) == 0,"","and  平台 in ('" + 平台 + "')")} ${if(len(国家等级) == 0,"","and  国家等级 in ('" + 国家等级 + "')")} ${if(len(注册语言) == 0,"","and  注册语言 in ('" + 注册语言 + "')")} ${if(len(core) == 0,"","and  core in ('" + core + "')")}
)
select dt
     , period_type
     , period_week
     , 平台, 国家等级, 国家, 注册语言, user_type
     , user_value
     , chl
     , 渠道来源, core
     , source_chl
