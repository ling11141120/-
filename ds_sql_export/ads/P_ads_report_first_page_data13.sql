...
-- workflow_version : 21
-- create_user      : xixg
-- task_name        : ads_report_first_page_data13
-- task_version     : 1
-- update_time      : 2025-06-24 18:00:04
-- sql_path         : \starrocks\sch_ads_report_first_page\ads_report_first_page_data13
----------------------------------------------------------------
-- 前置SQL语句
truncate table ads.ads_report_first_page_data13;

-- SQL语句
INSERT INTO ads.ads_report_first_page_data13
select a.dt as create_date ,
       a.DAU,a.new_num 新增用户数,
       b.charge_num 充值人数,
       b.charge_money 充值金额,
...
from
    (
        ...
        where  dt>=date_sub(curdate(),interval 30 day)  and self_type = 5
          and   1 =1
        GROUP BY 1
...
left join (
     select dt,
            count(distinct user_id) charge_num,
