...
-- workflow_name    : tbl_ads_sv_series_similar_priority
-- workflow_version : 6
-- create_user      : chenmo
-- task_name        : ads_sv_series_similar_priority
-- task_version     : 3
-- update_time      : 2024-12-20 11:21:14
-- sql_path         : \starrocks\tbl_ads_sv_series_similar_priority\ads_sv_series_similar_priority
----------------------------------------------------------------
-- SQL语句
insert into ads.ads_sv_series_similar_priority
select
    book_id, mt, book_code2, days_max2, cost_sum2,
    roi_0to1_2, roi_0to3_2, roi_0to7_2,
    std_r0to7, std_r7to90, dpt0_rate2, new_amt_rate2, rt_rate2, ltv0_2, consume_rate_2, r0_std2, r7_std2,
    roi_0to1_diff, roi_0to3_diff, roi_0to7_diff, std_r0to7_diff, std_r7to90_diff, new_amt_rate_dpt_diff, dpt0_rate_diff,
    rt_rate_diff, ltv0_diff, consume_rate_diff, score
    ,row_number() over(partition by concat(book_id,mt) order by score) as rn
    ,now() as etl_tm
from (
    select a.book_id
        ,a.mt
        ,concat('【',b.language,'-',b.code,'】',b.book_id) as book_code2,a.current_language2 as current_language,b.current_language2
        ...
        ,(1-b.dpt0_rate)/b.r7_std  as std_r7to90
        ,b.dpt0_rate as dpt0_rate2
        ,b.new_amt_rate_dpt as new_amt_rate2    -- 修改 new_amt_rate →new_amt_rate_dpt
        ,b.rt_rate as rt_rate2
        ,b.ltv0 as ltv0_2
        ,b.consume_rate as consume_rate_2   -- 新增字段
        ,b.r0_std as r0_std2
        ,b.r7_std as r7_std2
        -- 对比数据，新增
    ...
    from (
        select
            *
        from ads.ads_sv_series_roi_growth_metrics
    ...
    left join (
        ...
        from ads.ads_sv_series_roi_growth_metrics
        where has_std=1
    ) b on a.current_language2=b.current_language2 and a.mt=b.mt and a.book_id<>b.book_id    -- 对于语言或英语
