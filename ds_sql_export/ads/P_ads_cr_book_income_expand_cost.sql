...
-- workflow_version : 18
-- create_user      : linq
-- task_name        : ads_cr_book_income_expand_cost
-- task_version     : 11
-- update_time      : 2024-10-16 11:54:57
-- sql_path         : \starrocks\tbl_ads_sr_book_income_expand_di\ads_cr_book_income_expand_cost
----------------------------------------------------------------
-- 前置SQL语句
delete from ads.ads_cr_book_income_expand_cost where dt>=date_sub('${dt}',interval 3 day ) and dt<date_sub('${dt}',interval 2 day );

-- SQL语句
insert into ads.ads_cr_book_income_expand_cost
with inex as(
    select dt,book_id,
    ...
    from(
        ...
        from (
            ...
            where a.create_time >= date_sub('${dt}',interval 3 day ) and a.create_time < date_sub('${dt}',interval 2 day ) and
            ...
            inner join dwd.dwd_cr_trade_original_recharge_view b
               on a.coo_order_id = b.out_trade_no
            where a.refund_time>= date_sub('${dt}',interval 3 day ) and a.refund_time < date_sub('${dt}',interval 2 day ) and
        ...
        from(
            ...
            from(
                ...
                where a.create_time>=date_sub('${dt}',interval 3 day ) and a.create_time<date_sub('${dt}',interval 2 day ) and a.product_id=6773 and tv_id is not null
                group by 1,2
            )t1
            where cn_expand>0
            union all
...
select inex.dt,
       ...
       inex.book_id,b.original_id,inex.cn_income_amt as income2,cr_coef,inex.cn_income_amt*cr_coef as cn_income,inex.cn_expand,inex.cn_distribution_expand,now() as etl_time
from inex
left join dim.dim_cr_book_da_view b on inex.book_id=b._id
left join (
    select story_id,cost_rate/100 as cr_coef
    from dim.dim_sr_cp_story_view
    where product_type=2 and story_type=3 -- -------国阅------
)c on inex.book_id=c.story_id;
