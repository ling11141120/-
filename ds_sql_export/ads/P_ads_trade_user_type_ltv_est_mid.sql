...
-- workflow_name    : tbl_ads_trade_user_type_ltv_est_mid
-- workflow_version : 9
-- create_user      : hufengju
-- task_name        : ads_trade_user_type_ltv_est_mid
-- task_version     : 9
-- update_time      : 2026-01-26 16:08:26
-- sql_path         : \starrocks\tbl_ads_trade_user_type_ltv_est_mid\ads_trade_user_type_ltv_est_mid
----------------------------------------------------------------
-- SQL语句
insert into ads.ads_trade_user_type_ltv_est_mid
WITH pay_detail AS (
    select dt,Productid,userid,chargeitemcount,chargemoney
    from dws.dws_trade_user_shopitem_charge_est_ed where dt>=date_sub('${dt}',interval 120 day ) and dt<='${dt}'
    union all
    select dt,Product_id,user_id,charge_itemcount,charge_money
    from dws.dws_trade_short_viedo_payorder_est_ed where dt>=date_sub('${dt}',interval 120 day ) and dt<='${dt}'
    union all
    select a.dt,a.Product_id,user_id,a.charge_itemcount*7,charge_money_rmb
    ...
    where is_refund = 0 and is_sandbox = 0 and date_sub(settle_time, interval 13 hour) >= date_sub('${dt}', interval 120 day) and date_sub(settle_time, interval 13 hour) <= '${dt}'
),tmp as(
   ...
   where date(date_add(a.create_time,interval -13 hour)) >=date_sub('${dt}',interval 120 day ) and date(date_add(a.create_time,interval -13 hour))<='${dt}'
),acc as (
   select  dt,
