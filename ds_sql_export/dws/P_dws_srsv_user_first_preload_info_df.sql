...
from (
    select
       ...
       ,row_number() over(
            partition by product_id, user_id, ad_type
            order by
                create_time,
                case when upper(ad_format) = 'REWARDED' then 0 else 1 end
        ) as rn
    from (
        select
            a.product_id
        ...
        ) a on a.product_id = b.product_id and a.id = b.user_id
    ) t
) t
where rn = 1
