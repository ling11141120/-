...
from dim.dim_shuangwen_book_read_consume_info  t1
  ...
  left join (select
	...
	where t1.product_id not in (8858,6883,6833)
...
from a
   inner join ads.ads_bi_user_read_book_info  t1
   on t1.book_id = a.book_id
  left join dim.dim_dic dic_product  -- 产品名称
   on t1.product_id = dic_product.enum_id
   and dic_product.table_name = 'dwd_consume_user_consume_di'
   and dic_product.dic_column = 'product_id'
...
c as(
	...
	from a
	inner join   ads.ads_bi_user_book_consume_info t1
		on t1.book_id = a.book_id
	 left join dim.dim_dic dic_product  -- 产品名称
   on t1.product_id = dic_product.enum_id
   and dic_product.table_name = 'dwd_consume_user_consume_di'
...
group by 1,2
 ...
 on a.book_id = d0.book_id
   left join dim.dim_dic dic_product  -- 产品名称
   on d0.product_id = dic_product.enum_id
   and dic_product.table_name = 'dwd_consume_user_consume_di'
   and dic_product.dic_column = 'product_id'
   where 1=1
    	${if(len(产品名称) == 0,"","and  dic_product.enum_name in ( '" + 产品名称 + "')")}
...
e as (
    ...
    left join dim.dim_dic dic_product  -- 产品名称
   on a.product_id = dic_product.enum_id
   and dic_product.table_name = 'dwd_consume_user_consume_di'
   and dic_product.dic_column = 'product_id'
  ...
  ${if(len(是否引流) == 0 ,"",
  	...
  	and a.dt >= '${开始日期}'
  	and a.dt <= '${结束日期}'
	and a.book_id in (select book_id from a)
    ${if(len(产品名称) == 0,"","and  dic_product.enum_name in ( '" + 产品名称 + "')")}
