...
select
 ...
 SELECT product_id,
  ...
  expo_unt,
  cli_unt
from ads.ads_bi_book_itemexposure_info
where  product_id not in (6883, 6833)
	${if(len(书籍ID) == 0,"","and  book_id in ('" + 书籍ID + "')")}
...
where  t1.dt >= '${开始日期}' 	and t1.dt <= '${结束日期}'
	${if(len(书籍ID) == 0,"","and  book_id in ('" + 书籍ID + "')")}
      -- and is_channel_book = 0
    ${if(len(是否引流) == 0 ,"",
         "and case when  is_channel_book= 0 then '非引流' else null end in ('" + 是否引流 + "')")}
    and t1.corever = 1
	${if(len(CORE) == 0,"","and t1.corever in ('" + CORE + "')")}
...
e as (
  ...
  left join dim.dim_user_all_info d on a.user_id = d.user_id
  where a.product_id not in (6883, 6833)
...
left join c
on a.book_id = c.book_id and b.product_id = c.product_id
left join d
on a.book_id = d.book_id and b.product_id = d.product_id
left join e
on a.book_id = e.book_id and b.product_id = e.product_id
where
	1=1
