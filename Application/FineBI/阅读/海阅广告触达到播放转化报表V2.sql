...
case
	when t1.ad_position_id1 in ('18', '60', '69', '70', '71', '72', '57', '67', '5', '19', '16', '22') then concat(t2.ad_position_name,'(合并)')
...
select
	...
	ad_type,
	case
		when ad_position_id1 in ('18', '62') then '18'
		when ad_position_id1 in ('60', '64') then '60'
		when ad_position_id1 in ('69', '75') then '69'
		when ad_position_id1 in ('70', '76') then '70'
		when ad_position_id1 in ('71', '77') then '71'
		when ad_position_id1 in ('72', '78') then '72'
		when ad_position_id1 in ('57', '74', '79') then '57'
		when ad_position_id1 in ('67', '73', '80') then '67'
		when ad_position_id1 in ('5', '61') then '5'
		when ad_position_id1 in ('19', '63') then '19'
		when ad_position_id1 in ('16', '66') then '16'
		when ad_position_id1 in ('22', '65') then '22'
...
where
	...
	and project_id =5
	and cast(substring_index(app_version, '.', 1) as unsigned) * 100 + cast(substring_index(substring_index(app_version, '.', 2), '.', -1) as unsigned) * 10 + cast(substring_index(substring_index(app_version, '.', 3), '.', -1) as unsigned) >=197
