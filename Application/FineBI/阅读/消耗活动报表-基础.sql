...
from
		...
		from
			(
			  ...
			  ${if(len(group_id6) == 0,")","or  group_id in (" + group_id6 + ") )")}
              	${if(人群包来源 == "TAG人群包"," and 1=1 ","and 1=2 ")}
			  group by 1,2,3,4
                UNION  all
  select
     case
    ${"when SubCrowdId in (" + group_id1 + ")  then '实验组1' "}
    ${"when SubCrowdId in (" + group_id2 + ")  then '实验组2' "}
    ${"when SubCrowdId in (" + group_id3 + ")  then '实验组3' "}
    ${"when SubCrowdId in (" + group_id4 + ")  then '实验组4' "}
    ${"when SubCrowdId in (" + group_id5 + ")  then '实验组5' "}
    ${"when SubCrowdId in (" + group_id6 + ")  then '对照组' "}
    end group_type,
    ...
    row_number()  over(partition by ProductId,UserId order by min(createtime)) row_1
  ...
  where ProjectId = 1
    ...
    and createtime >= '${开始时间}'
    and createtime <= '${结束时间}'
    ${if(len(group_id1) == 0,"","and ( SubCrowdId in (" + group_id1 + ")")}
    ${if(len(group_id2) == 0,")","or  SubCrowdId in (" + group_id2 + ")")}
    ${if(len(group_id3) == 0,")","or  SubCrowdId in (" + group_id3 + ")")}
    ${if(len(group_id4) == 0,")","or  SubCrowdId in (" + group_id4 + ")")}
    ${if(len(group_id5) == 0,")","or  SubCrowdId in (" + group_id5 + ")")}
    ${if(len(group_id6) == 0,")","or  SubCrowdId in (" + group_id6 + ") )")}
	${if(人群包来源 == "北斗人群包"," and 1=1 ","and 1=2 ")}
  group by 1,2,3,4
			) t1
	left join
	...
	on t2.product_id=t3.product_id
      and t2.country=t3.short_name
      left join dim.dim_dic dic_lang  -- 注册/投放语言
      on t2.current_language2 = dic_lang.enum_id
...
select
	*,
	sum(case when group_type='对照组' then `group_uv` end) over(partition by min_day) group6_uv,
	sum(case when group_type='对照组' then `read_uv` end) over(partition by min_day) group6_read_uv
from
	(
	select
		group_type,
...
select
	*,
	sum(case when group_type='对照组' then `group_uv` end) over(partition by min_day) group6_uv,
	sum(case when group_type='对照组' then `consume_uv` end) over(partition by min_day) group6_consume_uv,
	sum(case when group_type='对照组' then `consume_amount` end) over(partition by min_day) group6_consume_amount,
	sum(case when group_type='对照组' then `coin_consume_amount_0` end) over(partition by min_day) group6_coin_consume_amount_0,
	sum(case when group_type='对照组' then `coin_consume_amount_1` end) over(partition by min_day) group6_coin_consume_amount_1
from
	(
	select
		group_type,
...
from z1
left join z2
on z1.group_type = z2.group_type and z1.min_day=z2.min_day