...
left join ads.ads_tidb_short_video_center_activity_position_view t2
...
select
    dt,
    case
        when dim_strategy.strategy_code regexp '^HC' THEN '活动'
    ...
    strategy_id,
    count(distinct case when if_third = 0 then user_id end ) `原生下单UV`,
    count(distinct case when if_third = 1 then user_id end ) `三方下单UV`
from ads.ads_sv_third_party_payment_funnel_create_order a
...
where
    ...
    ${if(len(是否订阅) == 0,"","and case
        when shop_item_type in ('SVIP','签到卡') then '订阅'
        when shop_item_type='普通充值' then '非订阅'
    end in ('" + 是否订阅 + "')")}
group by 1,2,3
),
...
select
    ...
    count(distinct case when if_third = 0 then user_id end ) `原生充值UV`,
    count(distinct case when if_third = 1 then user_id end ) `三方充值UV`,
    count(distinct case when if_third = 0 and subscribe_status in ('0','1') then user_id end ) `原生充值UV（剔除续订）`,
    count(distinct case when if_third = 1 and subscribe_status in ('0','1') then user_id end ) `三方充值UV（剔除续订）`,
    count(case when if_third = 0 then user_id end ) `原生充值PV`,
    count(case when if_third = 1 then user_id end ) `三方充值PV`,
    sum(case when if_third = 0 then time_duration end) `原生支付时长`,
    sum(case when if_third = 1 then time_duration end) `三方支付时长`,
    sum(base_amount) `充值金额`,
    sum(case when if_third = 0 then base_amount end) `原生充值金额`,
    sum(case when if_third = 1 then base_amount end) `三方充值金额`
from ads.ads_sv_third_party_payment_funnel_recharge a
left join dim_strategy
on a.strategy_id =  dim_strategy.id
where
    dt between '${开始时间}' and '${结束时间}'
...
inner join ads.ads_third_payment_config_view b
...
select
    ...
    count(distinct account) `入包UV`,
    count(user_id) `曝光PV`,
    count(distinct case when zfqd like '%三方%' then user_id end ) `三方曝光UV`
from ads.ads_sv_third_party_payment_funnel_exposure a
...
select
    ...
    name,
    strategy_code,
    concat(strategy_code,'：',name) `策略代号&名称`,
    `曝光UV`,
    `入包UV`,
    `曝光PV`,
    `三方曝光UV`,
