...
-- workflow_name    : tbl_dwm_ab_strategy_recharge_data_di
-- workflow_version : 8
-- create_user      : hufengju
-- task_name        : dwm_ab_strategy_recharge_data_di
-- task_version     : 8
-- update_time      : 2025-04-14 23:39:45
-- sql_path         : \starrocks\tbl_dwm_ab_strategy_recharge_data_di\dwm_ab_strategy_recharge_data_di
----------------------------------------------------------------
-- SQL语句
insert into dwm.`dwm_ab_strategy_recharge_data_di`
with
-- 充值数据与消费数据
t123 as(
	select t1.*
...
z1 as (
		select
			t3.dt ,
			case
				when t3.strategy_id is null then -99
			...
			case when t3.strategy_id is null then -99
	...
	*
	FROM
		...
		select
			...
			case
				when t2.strategy_id is null then -99
...
z3 as (
		select
			t4.dt ,
			case when t4.strategy_id is null then -99
