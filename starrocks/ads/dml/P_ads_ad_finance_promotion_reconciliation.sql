...
-- workflow_name    : tbl_ads_ad_finance_promotion_reconciliation
-- workflow_version : 2
-- create_user      : chenmo
-- task_name        : ads_ad_finance_promotion_reconciliation
-- task_version     : 2
-- update_time      : 2025-10-16 14:42:14
-- sql_path         : \starrocks\tbl_ads_ad_finance_promotion_reconciliation\ads_ad_finance_promotion_reconciliation
----------------------------------------------------------------
-- SQL语句
...
with fb_account as (
    ...
    join (
        select
            a.id, a.ad_id, a.ad_set_id, a.spend, date(a.date_start) as dt, a.country, a.fb_account_id,
    ...
    ) b on a.fb_account = b.fb_account_id
    group by b.dt, a.level2, a.fb_account, b.country, b.company_id, b.ad_optimizer_uid
),
ads_account as (
    -- AdsAccount
    select
    ...
    join (
        select
            a.id, a.ad_id, a.ad_set_id, a.spend, date(a.date_start) as dt, a.country, a.account,
            if(a.date_start >= b.BeginTime and a.date_start <= b.EndTime, b.CompanyId, 0) as company_id,
            c.ad_optimizer_uid,
            c.ads_optimizer
        from (
        ...
        on a.ad_id = c.ad_id
        where if(a.date_start >= b.BeginTime and a.date_start <= b.EndTime, b.CompanyId, 0) > 0
    ) c on b.AdGroupId = c.ad_id
    group by c.dt, a.level2, a.fb_account, c.country, c.company_id, c.ad_optimizer_uid
),
...
--        left join ods.ods_ads_tidb_sharpengine_ads_global_AdAccountAgent c on b.AgentId = c.Id
    ...
    join ods.ods_tidb_sharpengine_ads_global_TiktokAd b on a.fb_account = b.FbAccount
    join (
        select
            a.id, a.ad_id, a.ad_set_id, a.spend, date(a.date_start) as dt, a.country, a.account,
            if(a.date_start >= b.BeginTime and a.date_start <= b.EndTime, b.CompanyId, 0) as company_id,
...
third_ad_account_not4 as (
    ...
    from (
        ...
        from (
            select
                account, product_name, product_id, mt, core
    ...
    join dim.dim_ads_thirdadset_view b on a.fb_account = b.fb_account
    join (
        select
            a.id, a.ad_id, a.adset_id, a.spend, date(a.date_start) as dt, a.country, a.account,
            if(a.date_start >= b.BeginTime and a.date_start <= b.EndTime, b.CompanyId, 0) as company_id,
            c.ad_optimizer_uid,
            c.ads_optimizer
        from (
        ...
        on a.ad_id = c.ad_id
        where if(a.date_start >= b.BeginTime and a.date_start <= b.EndTime, b.CompanyId, 0) > 0
    ) c on b.adset_id = c.adset_id
    group by c.dt, a.level2, a.fb_account, c.country, c.company_id, c.ad_optimizer_uid, b.adset_id
),
...
select
    ...
    now() as etl_time
from (
    select * from fb_account union all
    select * from ads_account union all
    select * from tiktok_ads_account union all
    select * from third_ad_account_not4 union all
    select * from third_ad_account_is4
) a
left join ods.ods_ads_tidb_sharpengine_ads_global_AdAccountAgent b on a.level2 = b.Id
left join ods.ods_ads_tidb_sharpengine_ads_global_CompanyInfo c on a.company_id = c.Id
left join ods.ods_tidb_sharpengine_kpi_b_userinfo_tb d
on a.ad_optimizer_uid = d.Account and NickName not like '%离职%' and Delflag = 0 and DingUserId is not null
where company_id > 0;
