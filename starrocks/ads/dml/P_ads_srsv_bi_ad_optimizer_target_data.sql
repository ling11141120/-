...
with z1 as (
    select a.source_chl
         ...
         , case when a.dx = 1 then 1 else 0 end            as is_newad
         , e.book_id
         , e.ad_optimizer_uid
      ...
      join (select product_id
                 ...
                 , max_by(ad_optimizer_uid, cnt) as ad_optimizer_uid
                 , max_by(book_channel, cnt)     as book_channel
              from (select ad_set_id
                         , product_id
                         , book_id
                         , ad_optimizer_uid
      ...
      -- 优化师&组
      left join (select distinct a.code
                      , a.code_value
                      , left(a.code_value, 2) as group_product
                      , b.nick_name
                   from dim.dim_optimizer_groups_new_view as a
                   left join dim.dim_kpi_user_info_view as b
                     on a.code = b.account
                  where left(a.code_value, 2) in ('hj', 'hy')
                ) as d
        on e.ad_optimizer_uid = d.code
       and if(e.product_id = 6833, 'hj', 'hy') = d.group_product
...
, z3 as (
    select a.*
      ...
      left join (select book_id
                      , date_key
                      , max(if(mt = 1, r0_std, null)) as ios_r0_std
                      , max(if(mt = 4, r0_std, null)) as and_r0_std
      ...
      left join (select current_language2
                  ...
                  where book_channel = 1
                    and date_key > days_add(curdate(), -360)
                  group by 1, 2
                ) as put
        on a.languageid = put.current_language2
       and a.dt = put.date_key
      -- 海剧分剧标准
      left join (select date_key
        ...
        on a.source_chl = r2.source_chl
       and a.dt = r2.date_key
       and a.book_id = r2.video_id
      -- 海剧标准
      left join (select date_key
                      , current_language2
                      , source_chl
        ...
        on a.languageid = put2.current_language2
       and a.source_chl = put2.source_chl
       and a.dt = put2.date_key
)
...
, z5 as (
    select a.*
         ...
         , p.code_lv
         , p.test_status
         , date(p.begin_date)            as begin_date
         , date(p.end_date)              as end_date
         , first_value(nick_name) over (partition by concat(a.product, a.source2, a.book_id, a.core)
                                            order by case when ifnull(a.spend, 0) < 10 or nick_name like '%离职%' or nick_name is null then '2050-01-01' else dt end asc, nick_name
                                       ) as frt_nickname
         , first_value(nick_name) over (partition by concat(a.product, a.source2, a.book_id, a.core)
                                            order by case when ifnull(a.spend, 0) < 10 or nick_name like '%离职%' or nick_name is null then 0 else std_amt_all end desc, nick_name
                                       ) as nick_name_max
      from (select a.dt
                 , a.first_time
                 , a.weekdays
                 ...
                 -- 判断是否淘汰
                 , sum(case when b.is_newad = 1 and days_diff(a.dt, b.dt) < 7 and a.dt >= b.dt then b.spend end)                                                                                     as spend_10d
                 , sum(case when b.is_newad = 1 and days_diff(a.dt, b.dt) < 7 and a.dt >= b.dt then b.adset_num end)                                                                                 as adset_num_10d
              ...
              from (select *
                         , min(dt) over (partition by product, book_id) as first_time
                      from z4
                     where is_newad = 1
              ...
              left join z4 as b
               ...
               and a.ad_optimizer_uid = b.ad_optimizer_uid
     ...
     where a.core <= 1
       and if(days_diff(a.dt, p.update_time) >= 0, code_lv, '') not in('B')
       and if(days_diff(a.dt, p.update_time) >= 0, test_status, 0) < 2
)
-- 每日专属组标签
, book_group2 as (
    select product
         ...
         , min(ifnull(cycle_n, 0))                                                              as cycle_n_min
         , date(days_add(begin_date, least(ifnull(min(case when spend < cycle_n * spend_up then cycle_n end), 3), if(product = 1, 3, if(left(concat(current_language2, book_code), 4) = 'ENXY', 2, 1))) * 7)) as end_date2
      from (select a.product
              ...
              from (select product
                         ...
                         , begin_date
                         , dt
                         , max_by(ad_optimizer_group, spend_bf7d) as frt_group
                      from (select a.product
                                 , a.source2
                                 , a.current_language2
                                 , a.book_id
                              ...
                              left join z5 as b
                               ...
                               and a.book_id = b.book_id
                               and a.source2 = b.source2
                               and days_diff(a.begin_date, b.dt) between 0
                               and 7
              ...
              -- 获取专属组花费阈值
              left join dim.dim_srsv_ad_prduct_lang_rate as c
                on a.product = c.product
               and a.current_language2 = c.lang
 -- 优化组、投放花费，用于判断是否出专属期
              left join z4 as b
                on a.product = b.product
               and a.book_id = b.book_id
               and a.frt_group = b.ad_optimizer_group
               and a.source2 = b.source2
               and b.dt > a.begin_date
...
, frt_group as (
    select a.dt
         ...
         , date(days_add(case when (a.d0_amt_pow_old + a.d0_amt_pow) / (a.std_amt_pow_old + a.std_amt_pow) > 0.9 then null when a.days_10d >= if(a.product = 2, 3, 4) and a.adset_num_10d >= if(a.product = 2, 6, 10) and a.d0_amt_10d / a.std_amt_10d < if(a.source2 = 'meta', if(a.product = 2, 0.7, 0.65), 0.77) then a.dt else null end , 7) ) as off_enddate
         , now()                         as etl_tm
      from z5 as a
      -- 每日专属组标签
      left join book_group2 as b
        on a.product = b.product
       and a.source2 = b.source2
       and a.book_id = b.book_id
       and a.dt = b.dt
...
, z6 as (
    select dt
         ...
         , current_language
         , null                                        as nick_name
         , null                                        as ad_optimizer_uid
         , null                                        as ad_optimizer_group
         , null                                        as adset_num
         , null                                        as spend
         , null                                        as d0_amt
         , null                                        as std_amt
         ...
         , null                                        as reg_num_all
         , null                                        as reg_num_new
         , null                                        as regnum_new_7d
         , null                                        as regnum_all_7d
         , null                                        as spend_10d
         , null                                        as adset_num_10d
         , null                                        as days_10d
         , null                                        as d0_amt_10d
         , null                                        as std_amt_10d
         , null                                        as d0_amt_all
         ...
         , null                                        as d0_amt_pow
         , null                                        as std_amt_pow
         , null                                        as d0_amt_pow_old
         , null                                        as std_amt_pow_old
         , frt_nickname
         , nick_name_max
         , frt_group
         , is_frt_group
         , null                                        as off_begindate
         , null                                        as off_enddate
         , now()                                       as etl_tm
      from (select a.dt
                 , c.*
              ...
              join (select code_id
               ...
               and a.dt <= p.end_date
               and if(days_diff(a.dt, p.update_time) >= 0, code_lv, '') not in('B')
               and if(days_diff(a.dt, p.update_time) >= 0, test_status, 0) < 2 -- 进阶日前一天数据
              -- 书籍每日花费
              left join (select product
                ...
                on a.dt = b.dt
               and p.project_code = b.product
               and p.code_id = b.code_id
               and p.source2 = b.source2
              join (select product
                         , source2
                         , code_id
                         , code_stage
                ...
                on a.dt >= c.dt_min
               and if(a.dt <= c.dt_max, a.dt <= c.dt_max, (a.dt < c.end_date and a.dt > c.dt_max))
               and p.project_code = c.product
               and p.code_id = c.code_id
               and p.source2 = c.source2
