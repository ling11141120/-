...
with a as (
    select book_id
          ...
          ,p_cd_val                                         as language_id
          ,cd_val_desc                                      as language_name
          ,book_name
...
,c as (
     select dt
           ...
           ,sum(case when types = 1 and site_id =  333 then amount 
               ...
               ) / 100*7                                    as trad_money_total_amount
           ,sum(case when types = 1 and site_id != 333 then amount 
                     else null 
                 end
               ) / 100*7                                    as fore_money_total_amount
      ...
      where dt >= date_sub('${bf_1_dt}', interval dayofmonth('${bf_1_dt}') + 10 day)
        and dt <= '${bf_1_dt}'
      group by 1,2
)
-- 作者稿酬/翻译成本   p系列（国内编辑）
,d as (
    select date(a.daily_time)                                                           as dt
          ...
          ,sum(case when a.reward_type=1002 then a.remuneration_money else null end)    as p_1002
          ,sum(case when a.reward_type=1003 then a.remuneration_money else null end)    as p_1003
          ,sum(case when a.reward_type=1004 then a.remuneration_money else null end)    as p_1004
          ,sum(case when a.reward_type=1005 then a.remuneration_money else null end)    as p_1005
          ,sum(case when a.reward_type=1007 then a.remuneration_money else null end)    as p_1007
          ,sum(case when a.reward_type=1008 then a.remuneration_money else null end)    as p_1008
          ,sum(case when a.reward_type=1010 then a.remuneration_money else null end)    as p_1010
          ,sum(case when a.reward_type=1011 then a.remuneration_money else null end)    as p_1011
          ,sum(case when a.reward_type=1012 then a.remuneration_money else null end)    as p_1012
          ,sum(case when a.reward_type=1013 then a.remuneration_money else null end)    as p_1013
...
,e as (
    select date(a.daily_time)                                                           as dt
          ,a.book_id*1000+b.siteid                                                      as book_id
          ,0                                                                            as a_0000
          ,sum(case when a.reward_type=1002 then a.remuneration_money else null end)    as a_1002
          ,sum(case when a.reward_type=1003 then a.remuneration_money else null end)    as a_1003
          ,sum(case when a.reward_type=1004 then a.remuneration_money else null end)    as a_1004
          ,sum(case when a.reward_type=1005 then a.remuneration_money else null end)    as a_1005
          ,sum(case when a.reward_type=1007 then a.remuneration_money else null end)    as a_1007
          ,sum(case when a.reward_type=1008 then a.remuneration_money else null end)    as a_1008
          ,sum(case when a.reward_type=1010 then a.remuneration_money else null end)    as a_1010
          ,sum(case when a.reward_type=1011 then a.remuneration_money else null end)    as a_1011
          ,sum(case when a.reward_type=1012 then a.remuneration_money else null end)    as a_1012
          ,sum(case when a.reward_type=1013 then a.remuneration_money else null end)    as a_1013
      from dwd.dwd_trade_exclusiverewardnew_view    as a
      join dim.dim_zhangzhong_xzz_book_view         as b
        on a.book_id =(b.book_id-b.siteid)/1000
     where a.remuneration_status in (2 ,5) 
       ...
       and date(a.daily_time) < '${dt}'
       and a.daily_time >= '2024-03-01'
       and b.departmenttype=1 
     group by 1,2
     union all
    select date(date_add(date_add(str_to_date(concat(a.static_date, '01')
                                              ,'%y%m%d'
                                             )
                                  ,interval  1 month
                                 )
           ...
           ,concat(a.book_id,'090')                       as book_id
           ,sum(a.amount)                                 as a_0000
           ,0                                             as a_1002
           ,0                                             as a_1003
           ,0                                             as a_1004
           ,0                                             as a_1005
...
,f as (
    select date_sub(a.dt,interval 1 day)                       as dt
          ,a.book_id
          ,a.cn_font_length - b.cn_font_length                 as cn_publish_length
          ,a.proofread_length - b.proofread_length             as proofread_length
          ,a.publish_length - b.publish_length                 as publish_length
          ,case when a.site_id = 322 then ((a.proofread_length - b.proofread_length)/1000) * 61.50
                when a.site_id = 375 then ((a.proofread_length - b.proofread_length)/1000) * 42.50
                when a.site_id = 410 then ((a.proofread_length - b.proofread_length)/1000) * 50.50
                when a.site_id = 409 then ((a.proofread_length - b.proofread_length)/1000) * 47.50
                when a.site_id = 418 then ((a.proofread_length - b.proofread_length)/1000) * 49.50
                when a.site_id = 414 then ((a.proofread_length - b.proofread_length)/1000) * 52.50
                when a.site_id = 433 then ((a.proofread_length - b.proofread_length)/1000) * 52.50
                when a.site_id = 436 then ((a.proofread_length - b.proofread_length)/1000) * 26.50
                when a.site_id = 419 then ((a.proofread_length - b.proofread_length)/1000) * 26.50
                when a.site_id = 412 then ((a.proofread_length - b.proofread_length)/1000) * 57.50
                when a.site_id = 435 then ((a.proofread_length - b.proofread_length)/1000) * 47.50
                when a.site_id = 497 then ((a.proofread_length - b.proofread_length)/1000) * 49.25
                when a.site_id = 445 then ((a.proofread_length - b.proofread_length)/1000) * 50.50
                else 0
     ...
     where a.dt >=date(date_sub('${bf_1_dt}', interval dayofmonth('${bf_1_dt}') + 9 day ))
       and a.dt<= '${dt}'
     union all
    select update_time
          ,book_id
     ...
     where update_time >=date(date_sub('${bf_1_dt}', interval dayofmonth('${bf_1_dt}') + 9 day))  
)
-- 2024-07-10 v4新增的-针对繁体书籍的 在国内编辑系统平台  稿酬表下的 三方收益总收入
,third_income as (
    select date(a.create_time)                                 as dt
          ,a.book_id*1000+b.siteid                             as book_id
          ,sum(if(a.reward_type=1010,a.book_salle,0))          as 1010_income_amt                  
          ,sum(if(a.reward_type=1013,a.book_salle,0))          as 1013_income_amt                  
      from dwd.dwd_trade_exclusiverewardnew_view               as a
      join dim.dim_zhangzhong_xzz_book_view                    as b
        on a.book_id =(b.book_id-b.siteid)/1000
     where a.reward_type in (1010,1013)
       and a.create_time >=date_sub('${bf_1_dt}', interval dayofmonth('${bf_1_dt}') + 10 day)
...
,h as (
    select a.book_id
          ...
          ,b.book_name
      from (select a.swbook_id*1000+a.to_language                   as book_id
                  ,source_book
             ...
             union
            select swbook_id*1000+to_language
                  ,book_id source_book
              from dim.dim_book_shuangwen_en_objectbook_info_view
             where mod(book_id ,1000) in (449,90)
               and object_book_type =0
...
select g.dt                                                         as dt                          -- 日期
      ...
      ,if(e.book_id is not null ,'凤鸣轩稿酬已同步',null)             as fmx_flag                    -- 凤鸣轩书籍标识
