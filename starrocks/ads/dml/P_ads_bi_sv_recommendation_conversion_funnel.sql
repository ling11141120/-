...
-- 超点剧剧集清单
super_drama_series as (
    select distinct SeriesId as series_id
      from ods.ods_tidb_short_video_early_on_demand_series
     where Status = 1
),
-- 短剧曝光用户
t1 as(
	...
	max(is_super_drama) as is_super_drama
	from
		(
		select
			dt,
		...
		if(sds.series_id is not null, 1, 0) as is_super_drama,
	...
	from ads.ads_sensors_cd_video_operationpositionexposure_view a1
		...
		left join super_drama_series sds
		on split(a1.activity_link, '_')[8] = sds.series_id
		where
			dt >= '${bf_1_dt}'
			and dt <= '${dt}'
			and product_id = '6833'
...
select t1.dt
     , ifnull(t1.position_type, '-99')                                                                       as position_type
     , ifnull(t1.channel_id, '-99')                                                                          as channel_id
     , ifnull(t1.list_id, '-99')                                                                             as list_id
     , ifnull(t1.event_strategy_id, '-99')                                                                   as event_strategy_id
     , ifnull(t1.programme_id, '-99')                                                                        as programme_id
     , if(cast(t1.login_id as bigint(20)) is not null, t1.login_id, '-99')                                   as exp_user_id
     , ifnull(t1.core, '-99')                                                                                as core
     , ifnull(t1.mt, '-99')                                                                                  as mt
     , ifnull(t1.current_language2, '-99')                                                                   as current_language2
     , t1.channel_name
     , m1.Sort                                                                                               as channel_sort
     , t1.list_name
     , m2.Sort                                                                                               as list_sort
     , exp_cnt
     , t2.login_id                                                                                           as clc_user_id
     , clc_cnt
     , t3.login_id                                                                                           as watch_user_id
     , t3.watch_epis
     , t3.watch_all
     , t3.watch_percent_20
     , t3.watch_percent_50
     , t4.login_id                                                                                           as unlock_user_id
     , t4.all_unlock_epis
     , t4.unlock_epis
     , t4.unlock_amount
     , t4.all_epis
     , ifnull(t1.is_super_drama, 0)                                                                          as is_super_drama
  ...
  left join t2
   ...
   and t1.event_strategy_id = t2.event_strategy_id
   and t1.login_id = t2.login_id
   and t1.core = t2.core
   and t1.mt = t2.mt
   and t1.current_language2 = t2.current_language2
   and t1.programme_id = t2.programme_id
  left join t3
    on t2.dt = t3.dt
   and t2.position_type = t3.position_type
   and t2.channel_id = t3.channel_id
   and t2.list_id = t3.list_id
   and t2.event_strategy_id = t3.event_strategy_id
   and t2.login_id = t3.login_id
   and t3.core = t2.core
   and t3.mt = t2.mt
   and t3.current_language2 = t2.current_language2
   and t3.programme_id = t2.programme_id
  left join t4
    on t3.dt = t4.dt
   and t3.position_type = t4.position_type
   and t3.channel_id = t4.channel_id
   and t3.list_id = t4.list_id
   and t4.event_strategy_id = t3.event_strategy_id
   and t3.login_id = t4.login_id
   and t3.core = t4.core
   and t3.mt = t4.mt
   and t3.programme_id = t4.programme_id
  left join ods.ods_tidb_short_video_center_schema_channel m1
    on t1.programme_id = m1.SchemeId
   and t1.channel_id = m1.ChannelId
   and m1.DelStatus = 0
  left join ods.ods_tidb_short_video_center_channel_list m2
    on t1.channel_id = m2.ChannelId
   and t1.list_id = m2.ListId
   and m2.DelStatus = 0
  left join ads.ads_user_sv_subscribe_status_di sst
    on t1.login_id = sst.user_id
   and sst.dt = t1.dt
;
