...
-- task_name        : 海阅账龄汇总
-- task_version     : 2
-- update_time      : 2026-02-16 11:47:01
-- sql_path         : \starrocks\审计测试-总\海阅账龄汇总
----------------------------------------------------------------
-- 前置SQL语句
delete from ads.`ads_sr_finance_book_recharge_surplus_info` where aging_dt = last_day(date_sub('${dt}', interval 1 month));

-- SQL语句
insert into ads.`ads_sr_finance_book_recharge_surplus_info`
with recharge as (
    -- 用户充值明细
    select
        dt,
...
-- sql_path         : \starrocks\审计测试-总_改\海阅账龄汇总
----------------------------------------------------------------
-- 前置SQL语句
delete from ads.`ads_sr_finance_book_recharge_surplus_info` where aging_dt ='${last_day}';
