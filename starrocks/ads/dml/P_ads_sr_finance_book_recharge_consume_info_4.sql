----------------------------------------------------------------
-- 程序功能：海阅补充消耗主键id
-- 程序名：P_ads_sr_finance_book_recharge_consume_info_4.sql
...
from (
         select
             *,
             row_number() over (partition by user_id, book_id, amount, remain_amount_all, order_time) as rn
    ...
    where dt >= date_format(date_sub('${dt}', interval 1 month), '%Y-%m-01')
      and dt <= last_day(date_sub('${dt}', interval 1 month))
                   ...
                   on a.user_id = b.UserId and a.book_id = b.BookId
                       and if(a.book_id = 0 and b.Amount < 0, b.Amount, abs(a.amount)) = b.Amount and a.remain_amount_all = b.RemainAmount and a.order_time = b.CreateTime and a.rn = b.rn
;
