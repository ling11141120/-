...
SELECT  dt
       ,login_id
       ,ad_position_id
       ,if(ad_position_id in ('19','63','5','61','23','29'),programme_id, coalesce(ad_strategy_id,programme_id,-1))  as ad_strategy_id
       ,coalesce(main_strategy_id,-1) as main_strategy_id
       ,'曝光'     AS event
       ,'普通广告' as ad_type
       ,COUNT(1) AS pv
       ,0        AS amount
...
select
       ...
       ,coalesce(event_strategy_id,-1) as ad_strategy_id
       ,coalesce(main_strategy_id,-1) as main_strategy_id
       ,'曝光'     AS event
       ,'任务' as ad_type
       ,COUNT(1) AS pv
...
select
       ...
       ,if(ad_position_id in ('19','63','5','61','23','29'),programme_id, coalesce(ad_strategy_id,programme_id,-1))  ad_strategy_id
       ,coalesce(main_strategy_id,-1) as main_strategy_id
       ,'曝光'     AS event
       ,'H5广告' as ad_type
       ,COUNT(1) AS pv
...
select 
  ...
  login_id ,
  ad_position_id,
  if(ad_position_id in (19,63,5,61,23,29),programme_id,if(task_id is null,ad_strategy_id,coalesce(event_strategy_id,programme_id))) as ad_strategy_id,
  coalesce(main_strategy_id, -1) as main_strategy_id,
  '观看完成'  as  event,
  case when task_id is null then 'H5广告' when task_id is not null then '任务' end  as  ad_type,
  count(1) as pv,
  0 as amount  
...
select
       ...
       ,case when    positions in (19,63,5,61,23,29)  then      programme_id
          else if(event_strategy_id > 1,event_strategy_id,programme_id )  end as ad_strategy_id
       ,if(main_strategy_id = 'null' or main_strategy_id is null, -1, main_strategy_id) as main_strategy_id
       ,'收益'     AS event
       ,case when ad_show_type = '5' and  positions  = 59 then  '任务'
             when ad_show_type = '5' and  positions <> 59 then 'H5广告'
            else '普通广告' end as ad_type
       ,sum(cnt) AS pv
       ,SUM(amt) AS amount
...
(
        ...
        case
			...
			else  dic_mt.enum_name  end as mt,
...
select   
  ...
  ,ifnull(z1.ad_type,-99) as ad_type
  ...
  ,sum(if(event = '曝光' ,z1.pv,0)) as `impression_pv`
  ,sum(
     case when z1.event = '点击'  then z1.pv
          when z1.event = '收益' and  z1.ad_type in ('任务','H5广告') then z1.pv
          else 0 end  ) as `click_pv`
  ,sum(if(z1.event = '观看完成' ,z1.pv,0)) as `watch_completion_pv`
  ,sum(z1.amount) as `ad_revenue_amount`
  ,now() as etl_time
from z1
left join z2 on z1.dt=z2.dt and z1.login_id=z2.user_id
