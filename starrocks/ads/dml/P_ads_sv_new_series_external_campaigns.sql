...
-- workflow_name    : tbl_ads_sv_new_series_external_campaigns
-- workflow_version : 49
-- create_user      : chenmo
-- task_name        : ads_sv_new_series_external_campaigns
-- task_version     : 18
-- update_time      : 2025-12-01 15:12:15
-- sql_path         : \starrocks\tbl_ads_sv_new_series_external_campaigns\ads_sv_new_series_external_campaigns
----------------------------------------------------------------
-- 前置SQL语句
delete from ads.ads_sv_new_series_external_campaigns where project_id is not null;
...
select a.project_id,
       ...
       if(coalesce(a.is_plan_created, b.is_plan_created) = 0 and coalesce(b.is_toufang, a.is_toufang) = 1, null, -1) as rn,
       now() as etl_time
from (
    select
        2 as project_id,
        ...
        if((a.language = 3 and a.series_level <= 2) or (a.language != 3 and c.advanced_languages is not null), 1, ifnull(b.is_toufang, 0)) as is_toufang,
    ...
    from (
        select
            ...
            a.series_name,
            if(a.publish_status = 1, a.publish_status, if(c.IsScheduledPublish = 1, 4, -99)) as publish_status,
        ...
        where !(a.publish_status != 1 and c.ScheduledPublishTime is null) and ifnull(a.publish_edat, c.ScheduledPublishTime) is not null
        and a.series_name not like '%dj-%' and (((d.series_code is not null or a.language  = 2) and e.Status = 3) or f.IsSubtitles = 0) -- QC完成或该剧不需要字幕
    ) a
    left join (
        select
            code_id,
    ...
    ) b on a.series_id = b.code_id
    left join (
        -- 已进阶
        select
            c.series_code,
            concat('[', group_concat(json_object('language_id', b.language, 'spend', ifnull(c.spend, 0))), ']') as advanced_languages
        from (
            select
                code_id
            from ads.ads_srsv_ads_marketing_plan_view
        ...
        ) c on a.code_id = c.book_id
        group by c.series_code
    ) c on a.story_code = c.series_code
) a
left join ads.ads_sv_new_series_external_campaigns_user b on a.project_id = b.project_id and a.product_id = b.product_id and a.series_id = b.series_id;
...
-- workflow_version : 49
-- create_user      : chenmo
-- task_name        : english_rn
-- task_version     : 4
-- update_time      : 2025-03-05 16:27:34
-- sql_path         : \starrocks\tbl_ads_sv_new_series_external_campaigns\english_rn
----------------------------------------------------------------
-- SQL语句
-- 计算英语剧的 rn
insert into ads.ads_sv_new_series_external_campaigns
...
-- workflow_version : 49
-- create_user      : chenmo
-- task_name        : non_english_rn
-- task_version     : 5
-- update_time      : 2025-03-05 16:27:34
-- sql_path         : \starrocks\tbl_ads_sv_new_series_external_campaigns\non_english_rn
----------------------------------------------------------------
-- SQL语句
-- 计算非英语剧的 rn
insert into ads.ads_sv_new_series_external_campaigns
...
left join (
    ...
    left join (
        ...
        where id = 1
    ) b on a.series_title = b.parameter_name
) c on a.series_id = c.series_id;
