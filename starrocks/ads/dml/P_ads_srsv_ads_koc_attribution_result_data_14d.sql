...
------===============2025-08-01  新增广告收入 =============================
insert into ads.ads_srsv_ads_koc_attribution_result_data_14d
with attribution_user as(
    select a.product_id
        ...
        select product_id
          ...
          from (
            select product_id
                  ...
                  ,lead(begin_time, 1, '2099-01-01 00:00:00') over (partition by product_id, user_id order by begin_time, end_time) as end_time1
                  ,date_add(begin_time, interval 14 day) end_time2
              from dwd.dwd_srsv_advertisement_koc_attribution_record_view
             where begin_time >= '${bf_30_dt}'
             group by 1, 2, 3, 4, 5, 6, 7, 8
           ) a
       ) a
      left join (select product_id, id as user_id, create_time
      ...
      left join (select product_id, user_id, ad_id, date(begin_time) as dt, min(create_time) as create_time
            from dwd.dwd_srsv_advertisement_koc_attribution_record_view
           where begin_time >= '${bf_30_dt}'
          group by 1, 2, 3, 4
      ) d 
...
select * from (
    select a.dt 
          ...
          now() as etl_tm          
    from (select a.dt,a.product_id,a.ad_id,a.resource_id,a.user_id,a.mt,a.core,a.chl,a.current_language,a.koc_text,a.user_dt,
            ...
            0 as ad_amt,
            0 as ad_amt_14d
        from (select b.datestr as dt, a.product_id
                   ,a.user_id
                   ,a.resource_id
                   ,a.begin_time
                   ,a.end_time
                ...
                left join (select 6833 as product_id, series_id as book_id, language as languageid
             ...
             left join (select dt, ProductId as product_id, UserId as user_id
        ...
        from (select b.datestr as dt, a.product_id
            ...
            left join (select dt, create_tm, product_id
