----------------------------------------------------------------
-- 程序功能： 
-- 程序名： P_ads_srsv_ads_koc_attribution_result_data_14d
-- 目标表： ads.ads_srsv_ads_koc_attribution_result_data_14d
-- 负责人： 
-- 开发日期： 
----------------------------------------------------------------
...
select * 
  from (select a.dt
          ...
          from (select a.dt
                  ...
                  left join (select dt
                                   ...
                                   ,UserId                    as user_id
                                   ,cast(SUBSTRING_INDEX(SUBSTRING_INDEX(SUBSTRING_INDEX(SUBSTRING_INDEX(SUBSTRING_INDEX(packageid,'|',1),'Ps_Half_',-1
                                                                                                        ),'Ps_Shop_half_',-1
                                                                                        ), '_', 1
                                                                        ), '_', -1
                                                        ) as int
                ...
                select a.dt
                      ...
                      ,null    as order_id
                      ,case when c.create_tm < a.end_time then c.ad_amt
                            else 0
                        end    as ad_amt
                      ,case when c.create_tm < a.end_time_14d then c.ad_amt
                            else 0
                        end    as ad_amt_14d
                  from (select b.datestr                                                                         as dt
                              ,a.product_id
 ...
 where 1=1
   and a.dt >= '${bf_3_dt}'
   and ((a.dt>='2024-12-11' and a.dt<'2024-12-14') or a.dt>='2025-01-02')
   and core=1
;