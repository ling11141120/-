...
select a1.dt                           as dt
     , a1.usr_id                       as usr_id
     , a1.prj_type_cd                  as prj_type_cd
     , coalesce(a5.cd_val_desc, '-99') as prj_type_name
     , case when a1.prj_type_cd = 2 then a2.ttl_view_num else a3.ttl_view_num end as ttl_view_num
     , case when a1.prj_type_cd = 2 then a2.ttl_view_min else a3.ttl_view_min end as ttl_view_min
     , a1.pay_type                     as pay_mth
     , a4.is_sub                       as tp_prd
     , coalesce(a6.ad_show_cnt, 0)     as ad_show_cnt
     , case when coalesce(a6.ad_show_cnt, 0) > 0 then a6.ad_rev_amt / a6.ad_show_cnt else 0 end as avg_rev_per_ad
     , case when coalesce(a6.ad_show_cnt, 0) > 0 then (case
            when a1.prj_type_cd = 2 then a2.ttl_view_min
            else a3.ttl_view_min
        end) * 60 / a6.ad_show_cnt     as watch_sec_per_ad else 0 end as watch_sec_per_ad
  from (select b2.dt
          ...
          cross join (select user_id
                       ...
                       where begin_time >= date_sub(date_sub('${bf_1_dt}', interval 1 month), interval 1 year)
                     ) as c1
          -- KOC归因表（全量KOC用户，含无订单）
  ...
  left join (select b7.dt
                  ...
                  , case when b7.project_id = 8 then 2 else 1 end as prj_type_cd
                  , max(case when b7.subscription_days >= 1 then '是' else '否' end) as is_sub
               from ods_log.ods_sensors_cd_video_production_ordersuccess as b7
  ...
  left join (select b8.dt
                  ...
                  , case when b8.product_id = 6833 then 2 else 1 end as prj_type_cd
