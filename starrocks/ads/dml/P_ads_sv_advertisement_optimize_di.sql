...
(
    select book_info.series_id                                           as book_id
         ...
         , if(
               source_series_code like '%-%'
               , split_part(source_series_code, '-', 1)
               , source_series_code
           )                                                              as main_code
         ...
         , concat(
               upper(dic_booklang_en.cd_val)
               , '-'
               , book_info.source_series_code
           )                                                              as series_code
    from dim.dim_sv_series_hi book_info
        left join dim.dim_pub_code_mapping_dict dic_booklang
             on book_info.language = dic_booklang.cd_val
             and dic_booklang.app_plat = 'pub'
             and dic_booklang.cd_col = 'lang_cd'
        left join dim.dim_pub_code_mapping_dict dic_booklang_en
...
(
    ...
    from ads.ads_advertisement_fbadroiinstallreferrer_view a
