...
-- 活跃表
	...
	t2 as(
		select
			a.dt,a.create_time,a.user_id,a.corever2,dic_mt.enum_name,
			lpad(a.item_count, 3, '0') as item_count,
            ...
            when SPLIT(get_json_string(custom_data, '$.sendId'), '_')[1] is null then
            (case
            when split(get_json_string(custom_data, '$.activityLink'), '_')[1]=202100 and split(get_json_string(custom_data, '$.activityLink'), '_')[2] in (0,1) then '普通弹窗'
            when split(get_json_string(custom_data, '$.activityLink'), '_')[1]=202100 and split(get_json_string(custom_data, '$.activityLink'), '_')[2]=3 then '充值返回推弹窗'
            when split(get_json_string(custom_data, '$.activityLink'), '_')[1]=202100 and split(get_json_string(custom_data, '$.activityLink'), '_')[2] in (12) then '充值弹层'
            when split(get_json_string(custom_data, '$.activityLink'), '_')[1]=200800 and SPLIT(get_json_string(custom_data, '$.sendId'), '_')[2]='0'   then '解锁页VIP'
            ...
            else   split(get_json_string(custom_data, '$.activityLink'), '_')[1] end)
			...
			get_json_string(a.cooorder_extinfo, '$.SubscribeStatus') as SubscribeStatus,
	...
	left join (
        ...
        and is_remove=0
        group by 1, 2, 3
    ) b on SUBSTRING_INDEX(SUBSTRING_INDEX(SUBSTRING_INDEX(SUBSTRING_INDEX(SUBSTRING_INDEX(ExtInfo,'|',-1),'com.changdu.mobovideo.',-1),'com.changdu.moboshort.',-1),'com.changjian.moboshortcj.',-1),'third.',-1) = b.item_id and a.shop_item = b.shop_item_id
		left join dim.dim_dic  dic_mt  -- mt
	...
	t3b as(
		select
			...
			sum(amt) amt
		  from
		dws.dws_advertisement_user_position_amt_ed a
        left join dim.dim_dic  dic_mt  -- mt
        on a.mt = dic_mt.enum_id
      and dic_mt.table_name = 'dim_user_accountinfo_df'
      and dic_mt.dic_column = 'mt'
...
t4 as (
	...
	where  dt>='${bf_1_dt}' and dt<='${dt}'	 	and   app_id = 683001001
