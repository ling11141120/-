...
-- 开发日期：2025-10-13
----------------------------------------------------------------
insert into ads.ads_sv_third_party_payment_funnel_recharge_1
-- 活跃表
with active as(
    select t1.dt
          ,t1.user_id
...
payorder as(
    select dt
          ...
          ,case when SPLIT(get_json_string(custom_data, '$.sendId'), '_')[1]='201300' then '商店页'
                ...
                when SPLIT(get_json_string(custom_data, '$.sendId'), '_')[1] is null then (case when split(get_json_string(custom_data, '$.activityLink'), '_')[1]=202100
                ...
                when SPLIT(get_json_string(custom_data, '$.sendId'), '_')[1]='202100' and SPLIT(get_json_string(custom_data, '$.sendId'), '_')[2] in ('3') then '充值返回推弹窗'
                else '其他'
            end recharge_source
...
select payorder.dt
      ...
      ,case when strategy_id is null then '续订(或策略id为空)'
            ...
            when strategy_id in(119945781576073372,119945781576073373,119945781576073374,119945781576073375,119945781576073376,119945781576073377,119945781576073378
                               ,119945781576073379,119945781576073380,119945781576073381,119945781576073382,119945781576073383,119741098431744170,119741098431744171) then 'H5'
