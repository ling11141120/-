    ...
    from (select ad_set_id
          ...
          from (select ad_set_id                             as ad_set_id
                ...
                where source_chl in ('facebook','fbs2s','tt','tiktok app')
                  and create_time >= days_add('${dt}',-365)
...
-- 语言，代号，创编模板
   , z2 as (
    select a.source_chl                                        as source_chl
         ...
         ,COALESCE(t.template_name,t2.template_name)          as template_name
         ,COALESCE(t.template_id,t2.template_id)              as template_id
    from z1                                                  as a
            ...
            left join (select distinct a.task_id                                              -- 创编任务
                                 ,a.ad_set_id
                                 ,b.template_id                                                   -- 创编模板
                   ...
                   from (select distinct task_id    -- 创编任务
                         ...
                         where status =1 and IsUpgrade is not null
                        )                                           as a
           ...
           left join (select distinct a.task_id                                              -- 创编任务
                   ...
                   from (select distinct task_id    -- 创编任务
                                       ...
                                       ,IsUpgrade
                         from ads.ads_creation_ad_set_task_log_view
                         where status =1 and IsUpgrade is null
                        )                                           as a
                            left join ads.ads_creation_ad_set_task_view      as b
                                      on a.task_id=b.id
                            left join ads.ads_creation_template_view         as c
                                      on b.template_id=c.id
    ) t2  ON  (CASE WHEN a.is_spc IN ('SPC') THEN a.ad_camp_id ELSE a.ad_set_id END) = t2.ad_set_id
WHERE (t.ad_set_id IS NOT NULL OR t2.ad_set_id IS NOT NULL)
    ...
    from z2    as a1
                                            ...
                                            from ads.ads_bi_ad_cost_recharge_view                             as c4
                                                     left join dim.dim_pub_code_mapping_dict                           as c5
                                                               on c4.product_id = c5.cd_val
                                                                   ...
                                                                   and c5.app_plat = 'pub'
                                            where c4.dt between date_sub('${dt}', interval 6 day) and '${dt}'
             ...
             left join (select b7.ProjectCode
                        ...
                        where b7.AdsCreationAutoTaskId > 0
                          and b8.DateKey = date(date_sub('${dt}',interval 1 day))
...
-- 指标：标准，new占比，通过D0收入比例确定投放标准，昨日+今日
   , z3 as (
    select a.*
         ...
         ,case when a.product_id=6833 then coalesce(r2.mt0_r0_std,put2.mt0_r0_std)
               else coalesce(r.mt0_r0_std,put.mt0_r0_std)
        end                                                                                                      as mt0_r0_std
...
-- 淘汰参数：
   , z5 as (
    ...
    from (select a.source2
               ...
               ,a.old_d0_amt
               ,a.old_std_amt
               ,days_diff('${dt}',a.first_time)                                                                                                                                as days_book
               -- 新组 & 老组指标
               ,a.spend1
               ,a.d0_amt1
               ,a.std_amt1
