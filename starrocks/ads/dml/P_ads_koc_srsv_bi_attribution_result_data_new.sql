...
-- workflow_name    : tbl_ads_koc_srsv_bi_attribution_result_data_new
-- workflow_version : 15
-- create_user      : hufengju
-- task_name        : ads_koc_srsv_bi_attribution_result_data_new
-- task_version     : 11
-- update_time      : 2025-08-20 17:13:43
-- sql_path         : \starrocks\tbl_ads_koc_srsv_bi_attribution_result_data_new\ads_koc_srsv_bi_attribution_result_data_new
----------------------------------------------------------------
-- SQL语句
insert into ads.ads_koc_srsv_bi_attribution_result_data_new
--激活用户
with attribution_user as(
	select  a.product_id,a.dt,a.user_id,
	a.koc_text,if(minutes_diff(d.create_time,b.create_time)<1440,1,2) as user_type,ifnull(b.reg_country,-99) as reg_country,
	a.mt,a.chl,a.current_language,core,
	a.begin_time,a.end_time,a.resource_id,a.create_time,c.InstitutionId as institution_id,c.StarId as star_id
	from (
		select
			a.product_id,user_id,date(begin_time) as dt,begin_time,end_time,resource_id,koc_text,
	...
	left join (
		select product_id,id as user_id,create_time,reg_country
		from dim.dim_user_account_info_view
	  union all
		select product_id,user_id,create_time,reg_country
	...
	left join (
		select product_id,user_id,koc_text,date(begin_time) as dt,min(create_time) as create_time
...
active_user as (
	select
		a.product_id,a.dt,a.koc_text,a.user_type,a.reg_country,
...
(
	select a.dt,a.project_tp,a.user_id,3 as user_type,a.reg_country,b.koc_text,b.product_id,b.institution_id,b.star_id,b.mt,b.chl,b.current_language,b.core,b.book_id
	from (
		select dt,1 as project_tp,user_id,ifnull(reg_country,-99) as reg_country from dws.dws_user_wide_active_ed where dt>='${bf_3_dt}'
		union all
		select dt,2 as project_tp,user_id,ifnull(reg_country,-99) as reg_country from dws.dws_user_short_video_wide_active_ed where dt>='${bf_3_dt}'
	) a
	inner join
	(
			select *
		from (
			select  project_tp,user_id,3 as user_type,koc_text,product_id,a.create_time,c.InstitutionId as institution_id,c.StarId as star_id,
			a.mt,a.chl,a.current_language,core,c.DataId as book_id,
				row_number() over(partition by project_tp,user_id order by a.create_time desc ) rn
			from (
				select if(a.product_id=6833,2,1) as project_tp,a.product_id,user_id,3 as user_type,koc_text,a.create_time,
						cast(SUBSTRING_INDEX(SUBSTRING_INDEX(a.ad_id, 'Mt=', -1),'|',1) as int ) as mt,
						cast(SUBSTRING_INDEX(SUBSTRING_INDEX(a.ad_id, 'Chl2=', -1),'|',1) as string ) as chl,
						ifnull(c.languageid,cast(SUBSTRING_INDEX(SUBSTRING_INDEX(a.ad_id, 'CurrentLanguage2=', -1),'|',1) as int )) as current_language,
						cast(SUBSTRING_INDEX(SUBSTRING_INDEX(a.ad_id, 'Core=', -1),'|',1) as int ) as core
	...
	left join (
	select b.datestr as dt,if(a.product_id=6833,2,1) as project_tp,user_id
			from dwd.dwd_srsv_advertisement_koc_attribution_record_view a
...
(
	select a.dt,a.product_id,a.user_type,a.reg_country,a.koc_text,a.project_tp,a.institution_id,a.star_id,
			a.mt,a.chl,a.core,a.current_language,a.book_id,
...
,
koc_order as (
	select a.dt,a.product_id,a.koc_text,a.user_type,a.reg_country,a.resource_id,a.user_id,a.mt,a.core,a.chl,a.current_language,
				a.institution_id,
				a.star_id,
				a.user_dt,
				b.item_count,
			...
			from (
						select
							...
							a.institution_id,
							a.star_id,
							a.mt,a.core,a.chl,a.current_language,
