----------------------------------------------------------------
-- 程序功能： 海阅-用户书架每本书最后一次阅读时间
-- 程序名： P_ads_sr_user_book_last_read_time
-- 目标表： ads.ads_sr_user_book_last_read_time
-- 负责人： chenmo
-- 开发日期：2025-04-15
----------------------------------------------------------------
...
select a.product_id
  ...
  left join (select product_id
   ...
   and b.BookId = c.book_id
  left join dim.dim_user_account_info_view as d
    on a.product_id = d.product_id
   and a.user_id = d.id
  left join dim.dim_book_chapter_info      as e
    on b.BookId = e.book_id
   and c.chapter_id = e.chapter_id
  left join dim.dim_novel_book_info_view   as f
    on b.BookId = f.book_id
  left join dim.dim_book_chapter_info      as g
    on b.BookId = g.book_id
   and (e.serial_number + 1) = g.serial_number
