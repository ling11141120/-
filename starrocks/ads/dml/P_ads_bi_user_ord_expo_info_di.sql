...
select a1.dt             as dt
     ...
     , a1.ord_id         as order_id
     , a1.cur_pay_mth    as zffs
     , a1.lst_pay_mth    as last_zffs
     , a1.core           as core
     , a1.mt             as mt
     , a1.shopitem       as shop_item
     , a1.cstm_data      as custom_data
     , a1.sub_statu_cd   as subscribe_status
     , a1.reg_ctry_cd    as reg_country
     , a1.usr_type_name  as user_type
     , a1.lang_cd        as current_language2
     , a1.lang_name      as current_language2_name
     , a2.cur_zffs_list  as zffs_list
     , a2.next_zffs_list as next_zffs_list
     , now()             as etl_tm
from (
      select /*+ broadcast(c2, c3, c4) */
           ...
           , c1.shopitem
      from tmp_order c1
           left join dim.dim_short_video_user_accountinfo c2 
                  on c1.usr_id = c2.user_id
           left join dws.dws_user_short_video_wide_active_period_ed c3 
                  on c3.dt = '${bf_1_dt}' 
                 and c3.period_type = "ctt"
                 and c3.product_id = 6833 
                 and c1.usr_id = c3.user_id
           left join dim.dim_pub_code_mapping_dict c4 
                  on c4.app_plat = "pub" 
                 and c4.cd_col = "lang_cd" 
                 and c3.current_language2 = c4.cd_val
           left join dim.dim_pub_code_mapping_dict c5 
                  on c5.app_plat = "pub" 
                 and c5.cd_col = 'mt' 
                 and c1.mt = c5.cd_val
     where c1.dt = '${bf_1_dt}'
     ...
     left join (
                ...
                from ods.ods_tidb_short_video_payorder b1
                     left join tmp_recharge_exposure b2 
