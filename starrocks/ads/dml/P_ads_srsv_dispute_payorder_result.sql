...
with dispute_stripe as (
	select 
		...
		a.final_status		
	from  ods.`ods_cdmoney_report_tidb_cn_dispute_stripe` a 
	left join ods.`ods_cdmoney_report_tidb_cn_dispute_order_stripe` b on a.dispute_id=b.dispute_id
	left join (
	      SELECT
	        s1.product_id,
	        s1.user_id,
	      ...
	      FROM
	        (
	          ...
	          FROM
	            (
				  select dt,product_id,user_id,user_type from  dws.dws_user_short_video_wide_active_period_ed where period_type='ctt' group by 1,2,3,4
					union all
				  select dt,product_id,user_id,user_type from  dws.dws_user_wide_active_period_ed where period_type='ctt' group by 1,2,3,4
				) a 
				inner join (
						select product_id,user_id  from ods.`ods_cdmoney_report_tidb_cn_dispute_order_stripe` group by 1,2
							union all 
						select product_id,user_id  from ods.`ods_cdmoney_report_tidb_cn_dispute_order_paypal` group by 1,2
				) b on a.product_id=b.product_id and a.user_id=b.user_id 
	        ) s1
	      WHERE
	        s1.rn = 1
		) t1 on t1.user_id=b.user_id and t1.product_id=ifnull(b.product_id,left(b.pay_channel_id,4))
	...
	left join (
		select product_id,id as user_id,reg_country,current_language2  from dim.dim_user_account_info_view group by 1,2,3,4
		  union all
		select product_id,user_id,reg_country,current_language2 from dim.dim_short_video_user_accountinfo group by 1,2,3,4
	) t3 
		on t1.user_id = t3.user_id and t1.product_id=t3.product_id
		...
		(
	      ...
	      FROM
	        (
	          ...
	          FROM
	            dws.dws_user_market_channel_info_detail_td a 
				inner join (
					select product_id,user_id  from ods.`ods_cdmoney_report_tidb_cn_dispute_order_stripe` group by 1,2
						union all 
					select product_id,user_id  from ods.`ods_cdmoney_report_tidb_cn_dispute_order_paypal` group by 1,2
	      ...
	      WHERE
	        s1.rn = 1
	    ) t5 on ifnull(b.product_id,left(b.pay_channel_id,4))=t5.product_id and b.user_id=t5.user_id
) 
,
dispute_paypal as (
	select 
		...
		if(a.final_status in (6),1,0) is_status_succeed,
		a.final_status		
	from  ods.`ods_cdmoney_report_tidb_cn_dispute_paypal` a 
	left join ods.`ods_cdmoney_report_tidb_cn_dispute_order_paypal` b on a.dispute_id=b.dispute_id
	left join (
	      SELECT
	        s1.product_id,
	        s1.user_id,
