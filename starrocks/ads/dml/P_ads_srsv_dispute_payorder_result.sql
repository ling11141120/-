    ...
    select a.id
      ...
      left join (select s1.product_id
                    ...
                    from (select a.product_id
                                ,a.user_id
                                ,a.last_source
                                ,row_number() over (partition by a.product_id,a.user_id
                                                        order by a.install_date desc,a.mt asc,a.corever asc,a.lang2 asc
                                                   )    as rn
...
,dispute_paypal as (
    select a.id
          ...
          ,t5.source_chl
          ,if( a.update_time>='2025-06-24' and a.final_status in (5,7)
              ,a.update_time
              ,null
             )                                 as refund_time
