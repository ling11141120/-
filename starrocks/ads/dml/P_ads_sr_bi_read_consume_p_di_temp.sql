...
-- workflow_version : 20
-- create_user      : yanxh
-- task_name        : ads_sr_bi_read_consume_p_di_temp
-- task_version     : 6
-- update_time      : 2024-10-16 11:33:55
-- sql_path         : \starrocks\tbl_ads_sr_bi_read_consume_p_di\ads_sr_bi_read_consume_p_di_temp
----------------------------------------------------------------
-- SQL语句
insert overwrite ads.ads_sr_bi_read_consume_p_di_temp
         -- --------投放激活归因表------------
      with ins as (
  -- ------------------按产品，用户，激活时间，书籍id 开窗，以媒体类型降序以及激活时间升序取当天的第一条激活数据-------------------
   select dt,install_date,product_id,user_id,next_attribute_time,source,book_id,nick_name
    ...
    dwd.dwd_advertisement_adext_view b
    on a.ad_id=b.ad_id
    left join
    -- --通过工号关联员工信息表获取员工姓名（去重）
    ( select account ,nick_name  from     dim.dim_kpi_user_info_view
   qualify row_number() over(partition by account  order by last_login_time desc  ) =1 ) c
    on b.ad_optimizer_uid =c.account
   where a.dt>=date_sub('${bf_1_dt}',interval 7 day) and a.dt<'${dt}' and  a.product_id not in  (6833,6883,-1)  and a.book_id >0  and user_id>0 -- 过滤异常数据
      )  a
...
select   rd.dt,rd.product_id,rd.user_id,rd.book_id,rd.fst_read_tm,rd.mt,
         coalesce(
                 ...
                 when rd.product_id in (7757,8858,7777,8888) then  1 end
             ,c.languageid
         ) as lang_id,
   ...
   ( select id,hours_add(create_time,-13) as reg_time,product_id,mt from  dim.dim_user_account_info_view  ) b
    ...
    left join
     -- 获取书名，书籍代号
    (select book_id,book_name,book_code,languageid from  dim.dim_shuangwen_book_read_consume_info )  c
     on rd.book_id=c.book_id
  where rd.dt>=date_sub('${bf_1_dt}',interval 7 day) and rd.dt<'${dt}'
  --  and rd.book_id=   7970409    and rd.mt=1  and rd.dt='2024-07-04';
