----------------------------------------------------------------
-- 程序功能： BI-海剧海阅移动终端Push监控
-- 程序名： P_ads_srsv_bi_mt_push_mon_dml
-- 目标表： ads.ads_srsv_bi_mt_push_mon
-- 负责人： qhr
-- 开发日期：2025-10-21
-- 版本号： v0.0.0
----------------------------------------------------------------

-- ${bf_1_dt}：固定传入T-1日期的yyyy-MM-dd 00:00:00
-- ${dt}：传入当前调度时间的yyyy-MM-dd HH:00:00
insert into ads.ads_srsv_bi_mt_push_mon
select '${dt}'                                                                    as stat_time
      ,6833                                                                       as product_id
      ,a1.corever                                                                 as core
      ,a1.mt                                                                      as mt
      ,count(a2.Id)                                                               as svr_push_tsk_num
      ,bitmap_union(to_bitmap(get_json_string(a2.Body, '$.custom.accountId')))    as svr_push_uv
      ,sum(case when a2.IsSuccess = 1 then 1 else 0 end)                          as svr_push_succ_tsk_num
      ,bitmap_union(to_bitmap(case when a3.login_id is not null then a1.unique_cdreader_id 
                                   else null
                               end
                             )
                   )                                                              as push_cli_arr_dev_num
      ,bitmap_union(to_bitmap(a4.account_id))                                     as cli_push_uv
      ,bitmap_union(to_bitmap(a5.login_id))                                       as cli_clk_uv
      ,null                                                                       as cli_dau
      ,bitmap_union(to_bitmap(a4.active_user_id))                                 as cli_push_act_uv
  from dim.dim_short_video_user_accountinfo                          as a1
  left join ods.ods_tidb_unifypush_log_log_pushlog_sv                as a2
    on a2.dt = case when hour('${dt}') = 0 then date('${bf_1_dt}') else date('${dt}') end
   and a2.create_time >= case when hour('${dt}') = 0 then '${bf_1_dt}' else '${dt}' end
   and a2.create_time < '${dt}'
   and (a2.AppId % 2 = 1 or a2.AppId % 2 = 0)
   and a1.user_id = bitmap_union(to_bitmap(get_json_string(a2.Body, '$.custom.accountId')))
  left join ods_log.ods_sensors_cd_video_pushdelivery                as a3
    on a3.dt = case when hour('${dt}') = 0 then date('${bf_1_dt}') else date('${dt}') end
   and a3.event_tm >= case when hour('${dt}') = 0 then '${bf_1_dt}' else '${dt}' end
   and a3.event_tm < '${dt}'
   and a3.app_product_id = 6833
   and a1.user_id = a3.login_id
  left join ads.ads_center_push_position_message_di_analysis_json    as a4
    on a4.dt = case when hour('${dt}') = 0 then date('${bf_1_dt}') else date('${dt}') end
   and a4.need_to_send_time >= case when hour('${dt}') = 0 then '${bf_1_dt}' else '${dt}' end
   and a4.need_to_send_time < '${dt}'
   and (a4.send_status = 1 or a4.push_position_id in (1,2))
   and coalesce(a4.mt,-99) in (1, 4)
   and a1.user_id = a4.active_user_id
  left join ads.ads_sensors_video_pushclick_view                     as a5
    on a5.dt = case when hour('${dt}') = 0 then date('${bf_1_dt}') else date('${dt}') end
   and a5.event_tm >= case when hour('${dt}') = 0 then '${bf_1_dt}' else '${dt}' end
   and a5.event_tm < '${dt}'
   and a1.user_id = a5.login_id
  -- left join dws.dws_srsv_wide_user_type_info_di                      as a6    -- fix
  --   on a6.dt = case when hour('${dt}') = 0 then date('${bf_1_dt}') else date('${dt}') end
 where coalesce(a1.mt,-99) in (1, 4)
   and a1.corever is not null
;