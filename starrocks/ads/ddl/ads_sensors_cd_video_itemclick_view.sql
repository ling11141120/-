create or replace view ads.ads_sensors_cd_video_itemclick_view (
     dt                    comment "日期"
    ,id                    comment "主键"
    ,rid                   comment "记录ID"
    ...
    ,element_id            comment "控件ID：当页面有下钻控件时上报，目前榜单集合/浏览历史/我的追剧"
    ,element_name          comment "控件名称：当页面有下钻控件时上报，目前榜单集合/浏览历史/我的追剧"
    ,shortplay_id          comment "短剧ID：整部短剧的ID"
    ,is_shortplayshelf     comment "短剧是否已在追剧中：若短剧已主动加入追剧页面记录为1-是，不在为0-否"
    ,element_type          comment "控件类型：只有运营位为控件或特殊对象时记录，例如：普通弹窗、底部弹窗等"
    ,channel_id            comment "频道id：在首页里不同频道分页id"
    ,channel_id1           comment "新频道id"
    ,channel_name          comment "频道名称：在首页里不同频道分页名称：找剧 /发现"
    ,list_id               comment "榜单id：从后台获取榜单id"
    ,list_id1              comment "新榜单id"
    ,send_id               comment "转换来源"
    ,list_style            comment "榜单样式：从后台获取榜单id的“榜单样式”"
    ,list_name             comment "榜单名称：热播短剧/主编力荐/人气排行等"
    ,list_index            comment "榜单位序：用户看到的榜单信息中的位序"
    ,activity_id           comment "活动id"
    ,event_strategy_id     comment "策略id"
    ,group_id              comment "用户分组id"
    ,keyword               comment "搜索词：搜索结果页面上报"
    ,search_mode           comment "搜索方式：{1：主动搜索，2：猜你想搜，3：历史搜索} 根据上次搜索方式上报，搜索结果页面上报"
    ,search_hit_type       comment "搜索命中方式"
    ,project_id            comment "5阅读 8 短剧"
    ,label_type            comment "角标类型"
    ,list_class            comment "榜单分类"
    ,etl_tm                comment "清洗时间"
)
comment "event=itemClick 短剧 点击事件"
as
select a1.dt                                      as dt                             -- 日期
      ...
      ,a1.element_id                              as element_id                     -- 控件ID：当页面有下钻控件时上报，目前榜单集合/浏览历史/我的追剧
      ,a1.element_name                            as element_name                   -- 控件名称：当页面有下钻控件时上报，目前榜单集合/浏览历史/我的追剧
      ,a1.shortplay_id                            as shortplay_id                   -- 短剧ID：整部短剧的ID
      ,a1.is_shortplayshelf                       as is_shortplayshelf              -- 短剧是否已在追剧中：若短剧已主动加入追剧页面记录为1-是，不在为0-否
      ...
      ,a1.channel_name                            as channel_name                   -- 频道名称：在首页里不同频道分页名称：找剧 /发现
      ,a1.list_id                                 as list_id                        -- 榜单id：从后台获取榜单id
      ,a1.list_id1                                as list_id1                       -- 新榜单id
      ,a1.send_id                                 as send_id                        -- 转换来源
      ,a1.list_style                              as list_style                     -- 榜单样式：从后台获取榜单id的“榜单样式”
      ,a1.list_name                               as list_name                      -- 榜单名称：热播短剧/主编力荐/人气排行等
      ,a1.list_index                              as list_index                     -- 榜单位序：用户看到的榜单信息中的位序
      ,a1.activity_id                             as activity_id                    -- 活动id
      ,a1.event_strategy_id                       as event_strategy_id              -- 策略id
      ,a1.group_id                                as group_id                       -- 用户分组id
      ,a1.keyword                                 as keyword                        -- 搜索词：搜索结果页面上报
      ,a1.search_mode                             as search_mode                    -- 搜索方式：{1：主动搜索，2：猜你想搜，3：历史搜索} 根据上次搜索方式上报，搜索结果页面上报
      ,a1.search_hit_type                         as search_hit_type                -- 搜索命中方式
  ...
  from ods_log.ods_sensors_cd_video_production_Itemclick    as a1
