DROP TABLE IF EXISTS ads.ads_srsv_ads_koc_attribution_result_data_log;
CREATE TABLE ads.ads_srsv_ads_koc_attribution_result_data_log (
    ...
    ,dev_unt               INT(11)                 COMMENT "激活用户数,不去重"
    ,koc_amt               DECIMAL(16,4)           COMMENT "充值金额"
    ,koc_amt_after         DECIMAL(16,4)           COMMENT "扣除渠道费之后的充值金额"
    ,order_num             INT(11)                 COMMENT "订单数"
    ,koc_amt_14d           DECIMAL(16,4)           COMMENT "订单14天充值金额"
    ,koc_amt_after_14d     DECIMAL(16,4)           COMMENT "订单14天真实收入金额"
    ,ad_amt                DECIMAL(12,2)           COMMENT "广告收入"
    ,etl_tm                DATETIME                COMMENT "清洗时间"
    ,batch_no              BIGINT(20)              COMMENT "批次号"
    ,create_time           DATETIME                COMMENT "日志创建时间"
    ,create_dt             DATE                    COMMENT "日志创建时间"
)
DUPLICATE KEY(dt, product_id, adid)
COMMENT "海阅海剧koc用户回收数据结果日志表"
