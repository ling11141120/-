CREATE EXTERNAL TABLE `ods_sn_starrocks_tb_sys_anr_info_external` (
  ...
  `scene` varchar(2048) NULL COMMENT "页面",
  ...
  `stack` varchar(65533) NULL COMMENT "主线程堆栈",
  `record_time` bigint(20) NULL COMMENT "日志保存时间",
  `system_info` varchar(65533) NULL COMMENT "ANR系统信息",
  `sdk_ver` varchar(64) NULL COMMENT "sdk版本	",
  `block_messages` varchar(65533) NULL COMMENT "阻塞消息(多条，json格式)",
  `trace_content` varchar(1048576) NULL COMMENT "",
...
COMMENT "日志总数表"
DISTRIBUTED BY HASH(`id`, `collect_time`) BUCKETS 20 
PROPERTIES (
"replication_num" = "3",
"in_memory" = "false",
