...
-- 2025-06-13 付费用户逻辑由订单数据dt<t-1 调整为dt<=t-1
-- -------------新用户按天的----------------------------
insert into dws.dws_srsv_wide_user_type_info_di
with tmp_recharge_user AS (
    -- 阅读的充值表-----------------
...
tmp_source as (
    select product_id,user_id,mt,corever,lang2,last_source from dws.dws_user_market_channel_info_detail_td where dt='${bf_1_dt}'
),
tmp_new_user as (
select
...
on a.productid=d.product_id and a.id=d.user_id
 ...
 where a.CreateTime >='${bf_1_dt}' and a.createtime<'${dt}' and a.productid  not in (3521,3531,6883) and cast(a.id as int)>0
),
...
select * from tmp_new_user
union all
select * from tmp_active_user
union all
select * from tmp_rmt_user