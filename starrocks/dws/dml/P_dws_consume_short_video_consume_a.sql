...
-- workflow_name    : tbl_dws_consume_short_video_consume_a
-- workflow_version : 4
-- create_user      : linq
-- task_name        : dws_consume_short_video_consume_a
-- task_version     : 4
-- update_time      : 2024-05-08 15:54:34
-- sql_path         : \starrocks\tbl_dws_consume_short_video_consume_a\dws_consume_short_video_consume_a
----------------------------------------------------------------
-- 前置SQL语句
delete from dws.dws_consume_short_video_consume_a where dt='${bf_1_dt}';
...
select dt,
       ...
       now() as etl_time
from(
    select '${bf_1_dt}' as dt,product_id,user_id,consume_amt_td,fst_consume_tm,lst_consume_tm,consume_cnt_td,consume_tv_td,consume_money_amt_td,
           fst_consume_money_tm,lst_consume_money_tm,consume_money_cnt_td,consume_money_tv_td,consume_cert_amt_td,fst_consume_cert_tm,
           lst_consume_cert_tm,consume_cert_cnt_td,consume_cert_tv_td
    from dws.dws_consume_short_video_consume_a
    where dt='${bf_2_dt}'
    union all
    -- 每日
    select dt,product_id,user_id,consume_amt_td,fst_consume_tm,lst_consume_tm,consume_cnt_td,consume_tv_td,consume_money_amt_td,
           fst_consume_money_tm,lst_consume_money_tm,consume_money_cnt_td,consume_money_tv_td,consume_cert_amt_td,fst_consume_cert_tm,
           lst_consume_cert_tm,consume_cert_cnt_td,consume_cert_tv_td
    from (
        ...
        if(min(if(consume_type=0,create_time,'9999-12-31'))<'9999-12-31',min(if(consume_type=0,create_time,'9999-12-31')),null)as fst_consume_money_tm,
        if(max(if(consume_type=0,create_time,'0000-01-01'))>'0000-01-01',max(if(consume_type=0,create_time,'0000-01-01')),null)as lst_consume_money_tm,
        ...
        if(min(if(consume_type=1,create_time,'9999-12-31'))<'9999-12-31',min(if(consume_type=1,create_time,'9999-12-31')),null)as fst_consume_cert_tm,
        if(max(if(consume_type=1,create_time,'0000-01-01'))>'0000-01-01',max(if(consume_type=1,create_time,'0000-01-01')),null)as lst_consume_cert_tm,
        ...
        from (
            select a.dt, a.id, 6833 as product_id, a.account_id as user_id, a.epis_id, b.series_id, a.epis_num, a.consume_type, a.consume_value, a.create_time
