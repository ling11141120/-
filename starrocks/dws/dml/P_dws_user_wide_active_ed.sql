...
from (
         ...
         from (  -- ---登录 交易 消耗 阅读事件 当作用户活跃主表----------------------
                  select dt,Productid, UserId from dwd.dwd_user_appstartlog where dt = '${bf_1_dt}' and UserId !=0
                  union all
                  select dt,Productid, UserId from dwd.dwd_trade_user_payorder  where dt = '${bf_1_dt}' and UserId !=0
                  union all
                  select dt,product_id, user_id from dwd.dwd_consume_user_consume where dt = '${bf_1_dt}' and user_id !=0
                  union all
                  select dt,Product_id, User_Id from dwd.dwd_read_user_chapter_view where dt = '${bf_1_dt}' and user_id !=0
              ) a
         group by 1,2,3
     ) b
left join (
...
from (
         select dt,Productid, UserId
         from (  -- ---登录 交易 消耗 阅读事件 当作用户活跃主表----------------------
                  select dt,Productid, UserId from dwd.dwd_user_appstartlog where dt = '${dt}' and UserId !=0
                  union all
                  select dt,Productid, UserId from dwd.dwd_trade_user_payorder  where dt = '${dt}' and UserId !=0
                  union all
                  select dt,product_id, user_id from dwd.dwd_consume_user_consume where dt = '${dt}' and user_id !=0
                  union all
                  select dt,Product_id, User_Id from dwd.dwd_read_user_chapter_view where dt = '${dt}'  and user_id !=0
              ) a
         group by 1,2,3
     ) b
left join (
