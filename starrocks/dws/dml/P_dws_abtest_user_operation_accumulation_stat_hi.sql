...
-- workflow_name    : tbl_dws_abtest_user_operation_accumulation_stat_hi
-- workflow_version : 11
-- create_user      : xixg
-- task_name        : dws_abtest_user_operation_accumulation_stat_hi__read
-- task_version     : 9
-- update_time      : 2024-06-03 18:23:13
-- sql_path         : \starrocks\tbl_dws_abtest_user_operation_accumulation_stat_hi\dws_abtest_user_operation_accumulation_stat_hi__read
----------------------------------------------------------------
-- SQL语句
-- 用户运营业务-- 阅读业务--- 累加指标小时统计
INSERT INTO dws.dws_abtest_user_operation_accumulation_stat_hi
-- 阅读业务-- 交易域---充值金额与充值次数指标统计
with read_trade_tmp AS (
    SELECT
        trade.dt_hour,
...
INNER JOIN(
            SELECT
                    ...
                    user_id,
                    recharge_amount,            -- 充值的金额（取分成前充值金额）
                    recharge_times              -- 充值次数
             FROM dws.dws_read_trade_user_trade_p_hi
             WHERE dt_hour >= '${dt}'
               AND dt_hour < '${af_1_dt}'
         )trade
  ON  exp_grp.user_id = trade.user_id
LEFT JOIN (
            SELECT
                project_id,
                exp_id,
...
INNER JOIN(
 ...
 ON exp_grp.user_id = consume.user_id
LEFT JOIN (
    SELECT
        project_id,
        exp_id,
...
SELECT dt_hour,
       ...
       exp_grp_type,                                                    -- 实验组类型
       exp_grp_name,                                                    -- 实验组名称
       MAX(traffic_allocation),                                              -- 流量占比
       SUM(recharge_times) AS recharge_times,                           -- 充值次数
       SUM(ifd_oper_recharge_amount) AS ifd_oper_recharge_amount,       -- 充值金额（交易域中分成前的充值金额）
       SUM(ifd_oper_money_consume) AS ifd_oper_money_consume,           -- 充值消费
       SUM(ifd_oper_total_consume) AS  ifd_oper_total_consume,           -- 总消费
       MAX(NOW())
FROM
        (SELECT dt_hour,
               project_id,                 -- 项目ID
               ...
               recharge_times,             -- 充值次数
               ifd_oper_recharge_amount,   -- 充值金额（交易域中分成前的充值金额）
               0 AS ifd_oper_money_consume,     -- 充值消费
               0 AS ifd_oper_total_consume      -- 总消费
        FROM read_trade_tmp
        UNION ALL
        SELECT dt_hour,
               project_id,                 -- 项目ID
               ...
               exp_grp_name,               -- 实验组名称
               traffic_allocation,         -- 流量占比
               0 AS recharge_times,             -- 充值次数
               0 AS ifd_oper_recharge_amount,   -- 充值金额（交易域中分成前的充值金额）
...
-- workflow_version : 11
-- create_user      : xixg
-- task_name        : dws_abtest_user_operation_accumulation_stat_hi__video
-- task_version     : 7
-- update_time      : 2024-06-03 18:23:13
-- sql_path         : \starrocks\tbl_dws_abtest_user_operation_accumulation_stat_hi\dws_abtest_user_operation_accumulation_stat_hi__video
----------------------------------------------------------------
-- SQL语句
-- 用户运营业务-- 海剧业务--- 累加指标小时统计
INSERT INTO dws.dws_abtest_user_operation_accumulation_stat_hi
-- 海剧业务-- 交易域---充值金额与充值次数指标统计
with video_trade_tmp AS (
    SELECT
        trade.dt_hour,
        exp_grp.project_id,                                                                         -- 项目ID
        exp_grp.exp_id,                                                                             -- 实验ID
...
-- 短剧业务-- 消耗域---充值消费与总消费指标统计
video_consume_tmp AS (
 SELECT
         consume.dt_hour,
         exp_grp.project_id,                                                                         -- 项目ID
         exp_grp.exp_id,                                                                             -- 实验ID
...
FROM
    (SELECT dt_hour,
     ...
     SELECT dt_hour,
            ...
            ifd_oper_money_consume,     -- 充值消费
            ifd_oper_total_consume      -- 总消费
     FROM video_consume_tmp
    )  a
GROUP BY 1,2,3,4,5,6,7;

----------------------------------------------------------------
-- project_name     : starrocks
-- workflow_name    : tbl_dws_abtest_user_operation_accumulation_stat_hi_yesterday
-- workflow_version : 6
-- create_user      : xixg
-- task_name        : dws_abtest_user_operation_accumulation_stat_hi__read
-- task_version     : 6
-- update_time      : 2024-06-03 21:05:50
-- sql_path         : \starrocks\tbl_dws_abtest_user_operation_accumulation_stat_hi_yesterday\dws_abtest_user_operation_accumulation_stat_hi__read
----------------------------------------------------------------
-- SQL语句
...
-- task_version     : 6
-- update_time      : 2024-06-03 21:05:50
-- sql_path         : \starrocks\tbl_dws_abtest_user_operation_accumulation_stat_hi_yesterday\dws_abtest_user_operation_accumulation_stat_hi__video
----------------------------------------------------------------
-- SQL语句
-- 用户运营业务-- 海剧业务--- 累加指标小时统计
INSERT INTO dws.dws_abtest_user_operation_accumulation_stat_hi
