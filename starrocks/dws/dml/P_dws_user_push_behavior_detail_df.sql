...
with p_info as (
    select push_id   as push_id
         ...
         , case when task_type = '3' then '章节更新' else '-' end as push_name
...
, send_view as (
    select '下发'          as event
      ...
      from (select b1.dt
                 ...
                 , b1.product_id
                 , case when b1.task_type is null and b2.push_id is not null then 4 else b1.task_type end as task_type
                 , 2             as msg_on
              from dwd.dwd_market_sr_push_msg_log_di as b1
              left join live_push_info as b2
...
, z_result as (
    select event
         , a1.dt
         , a1.id
         , if(event = '点击' , a1.task_type_name , a5.task_type_name ) as push_type
         , a1.push_id
         , if(event = '点击' , a1.push_name , a5.push_name )           as push_name
         , if(event = '送达' , case when a4.corever = 2 and a4.app_ver >= '3.9.5' then a1.user_id when a4.corever = 2 and a4.app_ver < '3.9.5' then -99 else a1.user_id end , a1.user_id ) as user_id
         , a4.corever
         ...
         , case when a4.app_notify = 1 then 1 else a1.msg_on end as app_notify_msg_on
         , a1.product_id
      from (select event
                 , dt
                 , id
...
select dt
     , ifnull(cast(product_id as int), -99) as product_id
     , event                  as event
     , id                     as id
     , ifnull(cast(push_id    as bigint), -99) as push_id
     , ifnull(cast(user_id    as bigint), -99) as user_id
     , ifnull(corever, -99)   as core
     , ifnull(mt, -99)        as mt
     , ifnull(push_type, -99) as push_type
     , ifnull(push_name, -99) as push_name
     , msg_on                 as msg_on
     , is_act                 as is_act
     , app_notify_msg_on      as app_notify_msg_on
     , now()                  as etl_tm
  from z_result
;
...
, z_result as (
    select a1.event
         , a1.dt
         , a1.id
         , case when a1.push_type is null and a4.p_type = 2 then 'live通知' else a1.push_type end as push_type
         , a1.push_id
         , if(a1.event = '点击' , case when a1.push_id = '101' and a1.push_type = '1' then '本地PUSH-定时签到' when a1.push_id = '101' and a1.push_type = '2' then '本地PUSH-沉默召回' when a1.push_id = '101' and a1.push_type = '3' then '本地PUSH-解锁屏幕' else a4.push_position_name end , a4.push_position_name ) as push_name
         , a1.user_id
         , a5.corever
         ...
         , a1.msg_on
         , if(a2.user_id is not null, 1, 0) as is_act
         , case when a5.app_notify = 1 then 1 else a1.msg_on end as app_notify_msg_on
      from (select event
                 , dt
                 , id
                 , push_type
