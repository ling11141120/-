...
, rmt_user_type as (
    select a.dt
         ...
         , case when datediff(a.dt, b.install_dt) = 0 then 'D0'
                when datediff(a.dt, b.install_dt) >= 1 and datediff(a.dt, b.install_dt) <= 7
                    then 'D1-D7'
                when datediff(a.dt, b.install_dt) >= 8 and datediff(a.dt, b.install_dt) <= 30
                    then 'D8-D30'
...
select a.dt                        as dt
     ...
     , a.corever                   as corever
     , a.mt                        as mt
     , a.current_language          as current_language
     , a.current_language2         as current_language2
     , b.source_chl                as source_chl
     , a.reg_country               as reg_country
     , a.country_level             as country_level
     , a.reg_time                  as reg_time
     , a.reg_days                  as reg_days
     , a.sex                       as sex
...
, rmt as (
    ...
    select product_id       as product_id
     ...
     where user_period = 3
       and period_types = 1
       and stat_period >= date_sub('${dt}', interval 6 month)
       and stat_period <= '${dt}'
       and product_id = 6833
     group by 1, 2
)
