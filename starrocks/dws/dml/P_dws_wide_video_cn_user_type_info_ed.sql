...
-- workflow_name    : tbl_dws_wide_video_cn_user_type_info_ed
-- workflow_version : 3
-- create_user      : linq
-- task_name        : dws_wide_video_cn_user_type_info_ed
-- task_version     : 3
-- update_time      : 2023-11-16 17:36:50
-- sql_path         : \starrocks\tbl_dws_wide_video_cn_user_type_info_ed\dws_wide_video_cn_user_type_info_ed
----------------------------------------------------------------
-- 前置SQL语句
delete from dws.dws_wide_video_cn_user_type_info_ed where dt=${bf_1_dt}'';

-- SQL语句
insert into dws.dws_wide_video_cn_user_type_info_ed
...
from (
         ...
         left join (select 6883 as productid, unique_cdreader_id as id, create_time from dim.dim_video_cn_accountinfo_view) a2
                   on a1.product_id = a2.productid and a1.user_id = a2.id and a2.create_time < date_sub(a1.install_date, interval 1 HOUR)
