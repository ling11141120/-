...
-- workflow_name    : tbl_dws_user_short_video_wide_active_west5_ed
-- workflow_version : 2
-- create_user      : xixg
-- task_name        : dws_user_short_video_wide_active_west5_ed
-- task_version     : 2
-- update_time      : 2025-08-12 17:41:49
-- sql_path         : \starrocks\tbl_dws_user_short_video_wide_active_west5_ed\dws_user_short_video_wide_active_west5_ed
----------------------------------------------------------------
-- SQL语句
-- 脚本
...
with active_user_tmp as (
    ...
    from (
             -- 海外短剧登录表
             select DATE(date_add(create_time,INTERVAL -13 HOUR)) AS dt,product_id, user_id  from dwd.dwd_user_short_video_user_login_view  where dt >= '${bf_3_dt}' and dt<='${dt}' and user_id>=0
union all
-- 海外短剧支付表
select DATE(date_add(create_time,INTERVAL -13 HOUR)) AS dt,product_id ,user_id from dwd.dwd_trade_short_video_payorder
where dt>='${bf_3_dt}' and dt<='${dt}' and product_id =6833 and status=0 and test_flag =0 and user_id>=0
union all
-- 海外短剧消耗表
select DATE(date_add(create_time,INTERVAL -13 HOUR)) AS dt,6833 as product_id, account_id as user_id from dwd.dwd_sv_consume_user_consume_bill_pdi where dt >= '${bf_3_dt}' and dt<='${dt}' and account_id>=0
union all
-- 海外短剧观看记录表
select DATE(date_add(create_time,INTERVAL -13 HOUR)) AS dt,6833 as product_id,account_id as user_id from dwd.dwd_video_short_video_epis_history where date(create_time) >= '${bf_3_dt}' and date(create_time)<'${dt}'and account_id>=0
    ) a
...
select
    ...
    date_add(sacc.create_time,INTERVAL -13 HOUR) as create_time,
    sacc.sex,
    sacc.third_party_id,
    sacc.pass_word,
