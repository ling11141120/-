drop table if exists dws.dws_advertisement_ad_position_play_data_ed;
create table if not exists dws.dws_advertisement_ad_position_play_data_ed (
...
primary key(dt, ad_position_id, product_id)
comment "阅读-广告位置曝光点击播放数据"
distributed by hash(dt, ad_position_id, product_id) buckets 1
properties (
    "replication_num" = "3",
    "in_memory" = "false",
