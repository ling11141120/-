...
select a1.book_id
     , count(if(a1.free_chapter_num = 0, 1, null)) as free_chapter_num
     , now()                                       as etl_tm
  from (select (b1.book_id * 1000 + b1.site_id) as book_id
...
union all
        ...
        select (b1.book_id * 1000 + b1.site_id) as book_id
         ...
         where b1.productid = 3366
           and b1.site_id in (322, 436, 445, 435, 497, 495)
           and b1.status != -1
...
select a1.productid
     ...
     , if(a1.normalchapternum_f is null, '-', a1.normalchapternum_f)                                           as normal_chapter_num_f
     ...
     , a1.chapternum                                                                                           as total_chapter_num
     , ifnull(a1.speed_chapter_num, 0)                                                                         as speed_chapter_num
     , a1.isputdown
     , a1.putdownlevel
     , a1.penname
     , a1.authortype
     , now()                                                                                                   as etl_time
  from (select m.server_product_id   as productid
             , b1.bookid
             , date(b1.createtime)   as yt
             , b1.bookname
             , b1.createtime
             , b1.updatetime
             , case when b1.booknature is null then 0 else b1.booknature end as booknature
             , b1.signtype
             , b1.channel
             , b1.isfull
             , b1.siteid
             ...
             , b1.authorname
             , b2.latestupdatetime
             , b3.chaptercount       as chapternum
             , b3.speed_chapter_num
             , b4.free_chapter_num
             , b1.isputdown
             , b1.putdownlevel
             , b1.penname
             , b1.authortype
          from (select (c1.bookid * 1000) + c1.siteid as bookid
                     , c1.bookname
                     , c1.createtime
                     , c1.updatetime
                     , case when c2.authortype = 4 and c1.booknature not in (6, 9) then 4 else c1.booknature end as booknature
                     , c1.signtype
                     , c1.channel
                     , c1.isfull
                     , c1.siteid
          ...
          left join speed_chapter as b3
            on b1.bookid = b3.book_id
          -- ---获取书籍的超点章节数
          left join dim.dim_book_free_chapter_num_temp as b4
        ...
        select m.server_product_id   as productid
          ...
          from (select (c1.bookid * 1000) + c1.siteid as bookid
                  ...
                  where c1.productid = 3366
                   and c1.siteid in (322, 436, 445, 435, 497, 495)
               ) as b1
          left join dim.dim_rule_productid_lang_mapping as m
            on m.site_id = b1.siteid
          left join (select c1.productid
...
union all
        ...
        select '3333'                              as productid
             ...
             , case when b1.source in (1, 2) then 0 else 2 end as signtype
             , b1.channel
             , b1.isfull
             , b1.siteid
             , 333                                 as siteid2
             ...
             , b1.authorname
             , b1.latestupdatetime
             , b1.normalchapternum_f               as chapternum
             , b4.speed_chapter_num
             , b5.free_chapter_num
             , null                                as isputdown
...
union all
          ...
          left join (select c1.cid
            ...
            on b1.newcid = b3.cid
          left join speed_chapter as b4
            on b1.bookid = b4.book_id
          -- ---获取书籍的超点章节数
          left join dim.dim_book_free_chapter_num_temp as b5
            on b1.bookid = b5.book_id
          -- ---获取免费章节数
          left join (select c1.bookid * 1000 + c1.siteid as book_id
                          , c1.departmenttype
...
union all
        ...
        select coalesce(m.server_product_id, 0)                                                                                 as productid
             ...
             , null                                                                                                             as siteid
             , case a.Language when 1 then 777
                               when 2 then 333
                               else m.site_id
                end                                                                                                             as siteid2
             , null                                                                                                             as priceperthousand
