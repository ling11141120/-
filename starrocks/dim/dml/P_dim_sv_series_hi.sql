----------------------------------------------------------------
-- 程序功能： 短剧剧集维表(小时级)
-- 程序名： P_dim_sv_series_hi
-- 目标表： dim.dim_sv_series_hi
-- 负责人： qhr
-- 开发日期：2026-06-01
----------------------------------------------------------------

insert overwrite dim.dim_sv_series_hi
select a.SeriesId         as series_id
     ...
     , a.UnPublishedAt    as un_published_at
     ...
     -- 源剧
     , b.SeriesName       as source_series_name
     , b.Language         as source_series_language
     , b.SeriesCode       as source_series_code
     , b.CooperateType    as source_cooperate_type
     , b.RightsHolderId   as source_rights_holder_id
     , b.LangIds          as source_lang_ids
     , b.Coef             as source_coef
     -- 分类
     , c.series_type_ids
     , c.series_type_name
     -- 版权方
     , d.holder           as source_rights_holder_name
     , d.Type             as source_rights_holder_type
  ...
  left join (select srt.SeriesId
                  , array_agg(srt.SeriesTypeId) as series_type_ids
                  , array_agg(st.Name)          as series_type_name
               from ods.ods_tidb_short_video_series_ref_type  as srt
               left join ods.ods_tidb_short_video_series_type as st
                 on srt.SeriesTypeId = st.Id
              group by 1
            )                                            as c
