...
, language_dim as (
    select l.cd_val
      ...
      left join (select p_cd_val as lang_cd
                      , cd_val   as lang_abbr
                   from dim.dim_pub_code_mapping_dict
                  where app_plat = 'pub'
...
, series_attr as (
    select sv.seriesid      as series_id
         ...
         , case sv.serieslevel when 1 then 'S'
                               when 2 then 'A'
                               when 3 then 'B'
                               when 4 then 'C'
            end             as series_level_name
         , sv.worktype      as work_type
         , case sv.worktype when 1 then '男频'
                            when 2 then '女频'
                            when 3 then '双番'
            end             as work_type_name
         , ssv.localtype    as local_type
         , case ssv.localtype when 1 then '本土剧'
                              when 2 then '译制剧'
                              when 4 then '动漫'
            end             as local_type_name
         , ssv.localsubtype as local_sub_type
         , case ssv.localsubtype when 1 then '本土剧-AI短剧' end as local_sub_type_name
         , ssv.audiotype    as audio_type
         , case ssv.audiotype when 1 then '原声剧'
                              when 2 then '配音剧'
            end             as audio_type_name
         , ssv.dubbedtype   as dubbed_type
         , case ssv.dubbedtype when 1 then '人工配音'
                               when 2 then 'AI配音'
...
, series_type_agg as (
    select ref.seriesid                           as series_id
