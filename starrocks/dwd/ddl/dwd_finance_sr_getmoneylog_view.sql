create or replace view dwd.dwd_finance_sr_getmoneylog_view (
     dt             comment "GetTime 分区"
    ...
    ,cps
    ,chl2
    ,ChargeType     comment "充值类型,0:正常 100：测试 5：余额冻结（先把用户余额消费冻结了   之后用户解冻在getmoneylog 又会发放一次 ）"
    ,DeviceGUID     comment "当前用户的设备id"
    ,GiftMoney      comment "获得的礼券数量"
    ,TestFlag       comment "测试数据 1 是"
    ,ActualAmount   comment "订单真实充值金额，精准价格"
    ,snapshot_time  comment "快照时间"
    ,sr_createtime  comment "starrocks数据注入时间"
    ,sr_updatetime  comment "starrocks数据更新时间"
)
comment "财务海阅领卡获取到的阅币，礼券记录日志"
as
select a1.dt             as dt             -- GetTime 分区
      ...
      ,a1.cps            as cps
      ,a1.chl2           as chl2
      ,a1.ChargeType     as ChargeType     -- 充值类型,0:正常 100：测试 5：余额冻结（先把用户余额消费冻结了   之后用户解冻在getmoneylog 又会发放一次 ）
      ,a1.DeviceGUID     as DeviceGUID     -- 当前用户的设备id
      ,a1.GiftMoney      as GiftMoney      -- 获得的礼券数量
      ,a1.TestFlag       as TestFlag       -- 测试数据 1 是
      ,a1.ActualAmount   as ActualAmount   -- 订单真实充值金额，精准价格
  ...
  from ods.ods_tidb_finance_snapshot_readernovel_tidb_xx_getmoneylog    as a1
;
