create view `dwd_trade_sharpenginepaycenter_payorder_est_view`
            ...
            comment "充值数据"
as
select date(date_add(`ods`.`ods_tidb_sharpenginepaycenter_hk_payorder`.`CreateTime`, interval -13 hour)) as `dt`
     ...
     , date_add(`ods`.`ods_tidb_sharpenginepaycenter_hk_payorder`.`CreateTime`,
                interval -13 hour)                                                                       as `create_time`
     ...
     , cast((`ods`.`ods_tidb_sharpenginepaycenter_hk_payorder`.`GiveAmount` / 100) as
            DECIMAL64(10, 2))                                                                            as `give_amount`
     , cast((`ods`.`ods_tidb_sharpenginepaycenter_hk_payorder`.`BankAmount` / 100) as
            DECIMAL64(10, 2))                                                                            as `bank_amount`
     ...
     , cast((`ods`.`ods_tidb_sharpenginepaycenter_hk_payorder`.`BaseAmount` / 100) as
            DECIMAL64(10, 2))                                                                            as `base_amount`
