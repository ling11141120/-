...
select a.dt                                                         as dt              -- 分区日期
     ...
     , get_json_string( get_json_string(a.Body, '$.aps.attributes.extData')
                       ,'$.push_title_id'
                      )                                             as push_title_id   -- push标题ID
     , get_json_string( get_json_string(a.Body, '$.aps.attributes.extData')
                       ,'$.push_content_id'
                      )                                             as push_content_id -- push内容ID
...
select dt                                                        as dt
  ...
  from (select r.dt
             ...
             , get_json_string( get_json_string(r.Customers, '$.sensorsdata')
                               ,'$.push_title_id'
                              )                        as push_title_id
             , get_json_string( get_json_string(r.Customers, '$.sensorsdata')
                               ,'$.push_content_id'
             ...
             , now()                                   as etl_time
             , row_number() over (partition by r.dt, r.product_id, r.Id
